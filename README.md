...
Dashboard KPI widgets are configurable and reorderable. Available widgets include health score, open tasks, critical tasks, warnings, next task, completed tasks, due today, due this week, due this month, snoozed tasks, high-priority tasks, meter tasks, one-time tasks, upcoming week, notification status and unavailable tasks.

Version 1.7.0 expands the dashboard with quick-filter chips, pinned saved filters, compact density, default due filters, tag chips on task cards, problem-task selection and inverted selection.
