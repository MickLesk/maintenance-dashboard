...
- Card, compact, table, calendar and timeline dashboard views
...
The dashboard supports five persistent layouts:

| View | Purpose |
| --- | --- |
| Cards | Full maintenance cards with progress and actions |
| Compact | Dense list for larger task collections |
| Table | Comparable task fields in rows |
| Calendar | Tasks grouped by calculated due date |
...
Tasks can be selected individually or in groups. Supported bulk actions include completing, snoozing, changing category, area or priority, enabling, disabling, deleting and exporting selected tasks. Mutating bulk actions show a preview and can create an automatic safety backup before execution.

Dashboard KPI widgets are configurable and reorderable. Available widgets include health score, open tasks, critical tasks, warnings, next task, completed tasks, upcoming week, notification status and unavailable tasks.
