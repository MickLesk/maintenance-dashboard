...
from .inventory import normalize_budget
...
def normalize_settings(settings: dict[str, Any] | None) -> dict[str, Any]:
    ...
    dashboard["cost_tracking"] = bool(dashboard.get("cost_tracking", False))
    currency = str(dashboard.get("default_currency") or "EUR").strip().upper()
    dashboard["default_currency"] = currency if currency in SUPPORTED_CURRENCIES else "EUR"
    if dashboard.get("number_format") not in NUMBER_FORMATS:
        dashboard["number_format"] = "auto"
    if dashboard.get("number_format") not in NUMBER_FORMATS:
        dashboard["number_format"] = "auto"
    ...
    # Accept pre-v3 values so settings written by an older release still load.
    workflow["default_state"] = LEGACY_WORKFLOW_STATE_MAP.get(
        str(workflow.get("default_state") or "open"), "open"
    )
    if workflow["default_state"] not in {"open", "in_progress"}:
        workflow["default_state"] = "open"
    workflow.pop("default_recurrence_mode", None)
    workflow.pop("persistent_default_state", None)
    ...
    for name in MODULES:
        modules[name] = bool(modules.get(name, False))
    normalized["budget"] = normalize_budget(normalized.get("budget"))
    ...
    from .user_content import normalize_user_templates

    normalize_user_templates(normalized)
