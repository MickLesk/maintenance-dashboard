...
def build_runs(tasks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for task in tasks:
        ...
        for key in totals:
            value = max(0, int(stats.get(key) or 0))
            totals[key] += value
            if key in bucket:
                bucket[key] += value

    def skip_rate(completed: int, skipped: int) -> float:
        finished = completed + skipped
        return round(skipped / finished * 100, 1) if finished else 0.0

    for bucket in by_category.values():
        bucket["skip_rate"] = skip_rate(bucket["completed"], bucket["skipped"])
    return {
        **totals,
        "skip_rate": skip_rate(totals["completed"], totals["skipped"]),
