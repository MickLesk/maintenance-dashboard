...
def async_register_services(hass: HomeAssistant) -> None:
    ...
    async def mark_done(call: ServiceCall) -> None:
        await _manager(hass).async_mark_done(call.data["task_id"], call.data.get("done_at"), call.data.get("note"))
    ...
    async def send_digest(call: ServiceCall) -> None:
        await _manager(hass).async_send_notification(call.data.get("service"), call.data.get("message"))

    async def test_notification(call: ServiceCall) -> None:
        await _manager(hass).async_send_notification(call.data.get("service"), call.data.get("message") or "Maintenance Dashboard test notification")

    hass.services.async_register(
        DOMAIN,
        "mark_done",
        mark_done,
        schema=vol.Schema({vol.Required("task_id"): cv.string, vol.Optional("done_at"): cv.string, vol.Optional("note"): cv.string}),
    )
    hass.services.async_register(
        DOMAIN,
        "snooze",
    ...
    hass.services.async_register(
        ...
        "send_digest",
        send_digest,
        schema=vol.Schema({vol.Optional("service"): cv.string, vol.Optional("message"): cv.string}),
    )
    hass.services.async_register(
        DOMAIN,
        "test_notification",
