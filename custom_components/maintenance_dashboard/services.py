...
def async_register_services(hass: HomeAssistant) -> None:
    ...
    async def set_workflow_state(call: ServiceCall) -> None:
    ...
    async def reset_task_progress(call: ServiceCall) -> None:
        await _manager(hass).async_reset_task_progress(
            call.data["task_id"],
            note=call.data.get("note"),
        )

    async def restart_task_cycle(call: ServiceCall) -> None:
        await _manager(hass).async_restart_task_cycle(
            call.data["task_id"],
            restarted_at=call.data.get("restarted_at"),
            note=call.data.get("note"),
        )

    async def skip_task_cycle(call: ServiceCall) -> None:
        await _manager(hass).async_skip_task_cycle(
            call.data["task_id"],
            skipped_at=call.data.get("skipped_at"),
            note=call.data.get("note"),
        )
    ...
    async def process_notifications(call: ServiceCall) -> None:
        await _manager(hass).async_maybe_send_scheduled_notifications()

    async def cleanup_task_entities(call: ServiceCall) -> None:
        await _manager(hass).async_cleanup_task_entities()

    async def clear_notification_history(call: ServiceCall) -> None:
        await _manager(hass).async_clear_notification_history()

    async def create_backup(call: ServiceCall) -> None:
        await _manager(hass).async_create_backup(
            name=call.data.get("name"),
            pinned=call.data.get("pinned", False),
        )

    async def restore_backup_sections(call: ServiceCall) -> None:
        await _manager(hass).async_restore_backup_sections(
            call.data["backup_id"],
            sections=call.data.get("sections"),
            task_ids=call.data.get("task_ids"),
        )

    async def check_integrity(call: ServiceCall) -> None:
        await _manager(hass).async_check_integrity()

    async def repair_integrity(call: ServiceCall) -> None:
        await _manager(hass).async_repair_integrity()

    async def bulk_operation(call: ServiceCall) -> None:
    ...
    hass.services.async_register(
        ...
        schema=vol.Schema({
            vol.Required("task_id"): cv.string,
            vol.Optional("restarted_at"): cv.string,
            vol.Optional("note"): cv.string,
        }),
    )
    hass.services.async_register(
        ...
        schema=vol.Schema({
            vol.Required("task_id"): cv.string,
            vol.Optional("skipped_at"): cv.string,
            vol.Optional("note"): cv.string,
        }),
    )
    hass.services.async_register(
    ...
    hass.services.async_register(
        ...
        bulk_operation,
        schema=vol.Schema({
            vol.Required("task_ids"): vol.All(cv.ensure_list, [cv.string]),
