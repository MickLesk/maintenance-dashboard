...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _taskDetailSheetHtml() {
    ...
    const costsTab = `<section class="detail-section"><div class="detail-metrics"><div><span>${this._t("yearlyCosts")}</span><strong>${totalCost}</strong></div></div><div class="detail-timeline">${history.filter(event => event.type === "completed").map(event => {
      ...
      return `<article><ha-icon icon="mdi:cash"></ha-icon><div><strong>${Number(completion.cost || 0).toFixed(2)} ${this._html(completion.currency || this._defaultCurrency())}</strong><p>${this._html(completion.material || completion.note || "")}</p><small>${this._datetime(event.created_at)}</small></div></article>`;
  ...
  _taskYearCost(taskId) {
    ...
    return total ? `${total.toFixed(2)} ${this._defaultCurrency()}` : "—";
  },
});
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _styles() {
    ...
    .history-axis .history-row{position:relative}
