...
const EMPTY = {
  ...
  fixed_month: "9", fixed_day: "1", season: "autumn", tags: [], workflow_state: "planned", checklist: [],
...
const I18N = Object.freeze({
  "de": {
    ...
    "completionRequirementChecklist": "Checkliste muss vollständig sein",
    "completionRequirementCost": "Kosten sind Pflicht",
    "completionRequirementMaterial": "Material ist Pflicht",
    "completionRequirementNote": "Notiz ist Pflicht",
    "completionRequirementPerformedBy": "Ausführende Person ist Pflicht",
  ...
  "en": {
    ...
    "completionRequirementChecklist": "Checklist must be complete",
    "completionRequirementCost": "Cost is required",
    "completionRequirementMaterial": "Material is required",
    "completionRequirementNote": "Note is required",
    "completionRequirementPerformedBy": "Executor is required",
 ...
 * @property {"planned"|"in_progress"|"blocked"|"completed"} [workflow_state]
...
class MaintenanceDashboardPanel extends HTMLElement {
  constructor() {
    ...
    this._starterPacksCollapsed = false;
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _dashboardHtml() {
    ...
    return `
      ...
      <button class="dashboard-fab extended-fab" data-action="create" title="${this._t("newEntry")}" aria-label="${this._t("newEntry")}"><ha-icon icon="mdi:plus"></ha-icon><span>${this._t("newEntry")}</span></button>
    `;
  },
  ...
  _dashboardWidgetsHtml() {
    ...
    const items = [
      ...
      `<span><ha-icon icon="mdi:circle-outline"></ha-icon><b>${s.open ?? s.active ?? 0}</b> ${this._t("active")}</span>`,
      `<span class="${s.critical ? "critical" : ""}"><ha-icon icon="mdi:alert-circle-outline"></ha-icon><b>${s.critical ?? 0}</b> ${this._t("critical")}</span>`,
      `<span class="${s.warning ? "warning" : ""}"><ha-icon icon="mdi:alert-outline"></ha-icon><b>${s.warning ?? 0}</b> ${this._t("warnings")}</span>`,
      `<span><ha-icon icon="mdi:calendar-today-outline"></ha-icon><b>${this._countDueByFilter("today")}</b> ${this._t("dueToday")}</span>`,
      `<span><ha-icon icon="mdi:check-circle-outline"></ha-icon><b>${s.completed_this_year ?? 0}</b> ${this._t("completedThisYear")}</span>`,
      `<span><ha-icon icon="mdi:cloud-question-outline"></ha-icon><b>${s.unavailable ?? 0}</b> ${this._t("unavailable")}</span>`,
    ];
    ...
    return `<section class="timeline-view">${sorted.map(task => {
      ...
      return `<article class="timeline-entry ${status}">
        ...
        <div class="timeline-card" data-task-card="${this._html(task.id)}" style="--task-accent:${this._html(accent)}">
          <span class="icon-chip"><ha-icon icon="${this._html(task.icon || "mdi:wrench-clock")}"></ha-icon></span>
          <div class="timeline-main"><strong>${this._html(task.name)}</strong><small>${this._categoryLabel(task)} · ${this._scheduleSummary(task)}</small></div>
          ...
          <div class="timeline-actions"><button class="icon" title="${this._t("edit")}" data-edit="${task.id}"><ha-icon icon="mdi:pencil"></ha-icon></button>${status !== "completed" ? `<button class="icon" title="${this._t("done")}" data-done="${task.id}"><ha-icon icon="mdi:check"></ha-icon></button>` : ""}</div>
        </div>
      </article>`;
    }).join("")}</section>`;
  },
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _taskCard(task) {
    ...
    return `<article class="task-card ${status}" data-task-card="${this._html(task.id)}" style="--task-accent:${this._html(accent)}">
      ...
      <div class="workflow-strip"><span class="workflow-state state-${this._html(workflowState)}">${this._workflowStateLabel(workflowState)}</span>${showChecklist ? `<span class="workflow-metric">${this._t("checklist")}: ${checklistProgress.done}/${checklistProgress.total}</span>` : ""}</div>
      ...
      ${showChecklist ? `<div class="checklist-preview">${checklist.slice(0, 4).map((item, index) => `<label class="checklist-item ${item.done ? "done" : ""}"><input type="checkbox" data-toggle-checklist="${task.id}:${index}" ${item.done ? "checked" : ""} ${completed ? "disabled" : ""}><span>${this._html(item.label)}</span>${item.required ? `<em>${this._t("required")}</em>` : ""}</label>`).join("")}${checklist.length > 4 ? `<small>${this._t("checklistMore").replace("{count}", String(checklist.length - 4))}</small>` : ""}</div>` : ""}
  ...
  _templatesHtml() {
    ...
    const groupsHtml = order.map(key => `
      <section class="template-group">
        <div class="template-group-header"><div class="template-group-title"><span class="template-group-icon"><ha-icon icon="${this._categoryIcon(key)}"></ha-icon></span><div><p class="eyebrow">${this._t("category")}</p><h3>${this._t(key)}</h3></div></div><span>${grouped[key].length}</span></div>
    ...
    return `<section class="page-header"><div><p class="eyebrow">${this._t("brandName")}</p><h1>${this._t("templates")}</h1><p>${this._t("templateSelectHint")}</p></div></section>
      <section class="panel templates-workbench">
        ...
        <section class="template-filter-bar">
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _diagnosticsHtml() {
    ...
    return `<div class="dialog-backdrop"><section class="dialog wide diagnostics-dialog"><header><div><p class="eyebrow">${this._t("brandName")}</p><h2>${this._t("diagnostics")}</h2></div><button class="icon" data-action="close-diagnostics"><ha-icon icon="mdi:close"></ha-icon></button></header><div class="dialog-body">
  ...
  _notificationDialogHtml() {
    ...
    return `<div class="dialog-backdrop"><section class="dialog"><header><h2>${this._t("notifications")}</h2><button class="icon" data-action="close-notification-dialog"><ha-icon icon="mdi:close"></ha-icon></button></header><div class="dialog-body">
      <section class="dialog-section notification-overview">
        <div><p class="eyebrow">${this._t("brandName")}</p><h3>${this._t("notificationRules")}</h3><p class="section-hint">${this._t("notificationRulesHint")}</p></div>
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  async _saveGeneralSettings() {
    const patch = {
      ...
      workflow: {
        default_state: this.shadowRoot.getElementById("defaultWorkflowState")?.value || "planned",
        show_checklists: Boolean(this.shadowRoot.getElementById("workflowShowChecklists")?.checked),
        reset_checklist_on_completion: Boolean(this.shadowRoot.getElementById("workflowResetChecklist")?.checked),
        default_completion_requirements: {
          note: Boolean(this.shadowRoot.getElementById("workflowRequireNote")?.checked),
          material: Boolean(this.shadowRoot.getElementById("workflowRequireMaterial")?.checked),
          cost: Boolean(this.shadowRoot.getElementById("workflowRequireCost")?.checked),
          performed_by: Boolean(this.shadowRoot.getElementById("workflowRequirePerformedBy")?.checked),
          checklist: Boolean(this.shadowRoot.getElementById("workflowRequireChecklist")?.checked),
        },
      },
      native_platforms: {
        todo_enabled: Boolean(this.shadowRoot.getElementById("todoPlatformEnabled")?.checked),
        todo_include_disabled: Boolean(this.shadowRoot.getElementById("todoIncludeDisabled")?.checked),
        calendar_enabled: Boolean(this.shadowRoot.getElementById("calendarPlatformEnabled")?.checked),
        calendar_include_snoozed: Boolean(this.shadowRoot.getElementById("calendarIncludeSnoozed")?.checked),
        calendar_event_duration_minutes: Number(this.shadowRoot.getElementById("calendarEventDuration")?.value || 60),
      },
    };
    await this.hass.callWS({ type: "maintenance_dashboard/update_settings", patch });
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  _defaultWorkflowState() {
    ...
    return ["planned", "in_progress", "blocked"].includes(state) ? state : "planned";
  },
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _styles() {
    ...
    .settings-nav{display:flex;gap:8px;flex-wrap:wrap;margin:-8px 0 22px;}
    .settings-nav a{display:inline-flex;align-items:center;min-height:40px;padding:0 14px;border:1px solid var(--md-sys-color-outline-variant);border-radius:var(--md-sys-shape-corner-full);color:var(--md-sys-color-on-surface);text-decoration:none;font-weight:800;}
    .settings-nav a:hover{border-color:var(--md-sys-color-primary);background:color-mix(in srgb,var(--md-sys-color-primary) 12%,transparent);}
    ...
    .history-page-grid{display:grid;grid-template-columns:minmax(0,1fr) 300px;gap:22px;align-items:start;}
    .history-panel{border-radius:24px;background:transparent;border:0;padding:0;}
    .history-toolbar{grid-template-columns:minmax(260px,1fr) minmax(160px,190px) minmax(160px,190px) minmax(160px,220px);gap:12px;margin-bottom:16px;}
    .history-scope.segmented{grid-column:1/-1;justify-self:end;display:flex;gap:0;padding:3px;border:1px solid var(--md-sys-color-outline-variant);border-radius:var(--md-sys-shape-corner-large);background:var(--md-sys-color-surface-container-low);}
    ...
    .history-summary{position:sticky;top:24px;display:grid;gap:10px;padding:16px;border:1px solid var(--md-sys-color-outline-variant);border-radius:24px;background:var(--md-sys-color-surface-container);}
    .history-summary div{display:grid;grid-template-columns:40px 1fr auto;gap:10px;align-items:center;padding:8px;border-radius:16px;background:var(--md-sys-color-surface-container-low);}
    ...
    @media(max-width:1180px){
      ...
      .history-summary{position:static;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));}
    ...
    .templates-workbench{display:grid;gap:18px;padding:18px 20px;margin-bottom:22px}
    .templates-toolbar-main{display:grid;grid-template-columns:minmax(280px,1fr) auto;gap:16px;align-items:center}
    .template-selection-actions{display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap}
    .template-filter-bar{margin:0;display:grid;gap:14px}
    .category-tabs{display:flex;gap:8px;flex-wrap:wrap;overflow:visible}
    .category-tabs .tab{display:inline-flex;align-items:center;gap:8px;min-height:40px;padding:0 14px}
    .category-tabs .tab ha-icon{--mdc-icon-size:18px}
    .template-secondary-filters{justify-content:space-between}
    ...
    .settings-overview-strip{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px;margin:0 0 18px}
    .settings-overview-card{padding:16px 18px;border:1px solid var(--md-sys-color-outline-variant);border-radius:20px;background:var(--md-sys-color-surface-container)}
    .settings-overview-card span{display:block;color:var(--md-sys-color-on-surface-variant);font-size:.78rem;font-weight:800;text-transform:uppercase}
    .settings-overview-card strong{display:block;margin-top:10px;color:var(--md-sys-color-on-surface);font-size:1.08rem}
    .settings-overview-card small{display:block;margin-top:6px;color:var(--md-sys-color-on-surface-variant)}
    ...
    @media(max-width:1500px){.task-grid,.pack-strip,.settings-overview-strip{grid-template-columns:repeat(3,minmax(0,1fr))}}
    @media(max-width:1180px){.task-grid,.pack-strip,.settings-overview-strip,.settings-section-grid.two-column,.settings-section-grid.three-column{grid-template-columns:repeat(2,minmax(0,1fr))}.templates-toolbar-main{grid-template-columns:1fr}.template-selection-actions{justify-content:flex-start}.bulk-toolbar.floating-bulk-toolbar{bottom:96px}}
    @media(max-width:760px){.bulk-toolbar.floating-bulk-toolbar{left:16px;right:16px;bottom:86px}.pack-strip,.settings-overview-strip,.settings-section-grid.two-column,.settings-section-grid.three-column,.platform-grid{grid-template-columns:1fr}.settings-row{grid-template-columns:32px 28px minmax(0,1fr) 36px 36px;gap:8px}.settings-row [data-edit]{grid-column:4}.settings-row [data-delete]{grid-column:5}.settings-row [data-move$=':up'],.settings-row [data-move$=':down']{grid-row:2}.task-grid{grid-template-columns:1fr}}
