...
const EMPTY = {
  ...
  fixed_month: "9", fixed_day: "1", season: "autumn", tags: [], workflow_state: "open", checklist: [],
...
class MaintenanceDashboardPanel extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `${this._styles()}<main class="shell density-${this._html(this._density)}">${this._hero()}${content}${this._dialogHtml()}${this._taskDetailSheetHtml()}${this._qualityDialogHtml()}${this._templateImportDialogHtml()}${this._mobileActionSheetHtml()}${this._shortcutsDialogHtml()}${this._historyDialogHtml()}${this._diagnosticsHtml()}${this._dataDialogHtml()}${this._notificationDialogHtml()}${this._templatePreviewHtml()}${this._completionDialogHtml()}${this._bulkPreviewHtml()}${this._onboardingDialogHtml()}${this._toastHtml()}</main>`;
    this._bind();
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _taskCard(task) {
    ...
    return `<article class="task-card ${status}" data-task-card="${this._html(task.id)}" style="--task-accent:${this._html(accent)}">
      ...
      <div class="workflow-strip"><span class="workflow-state state-${this._html(workflowState)}">${this._workflowStateLabel(workflowState)}</span>${execution.sequence ? `<span class="workflow-metric">${this._t("runLabel")} ${execution.sequence}</span>` : ""}${showChecklist ? `<span class="workflow-metric">${this._t("checklist")}: ${checklistProgress.done}/${checklistProgress.total}</span>` : ""}</div>
