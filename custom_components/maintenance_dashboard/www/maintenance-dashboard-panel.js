...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _templatesSectionHtml() {
    ...
    return `<header><ha-icon icon="mdi:shape-outline"></ha-icon><div><h3>${this._t("templates")}</h3><p>${this._t("templateSelectHint")}</p></div><button class="ghost" data-action="open-template-import"><ha-icon icon="mdi:upload"></ha-icon>${this._t("importTemplates")}</button></header>
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _bind() {
    ...
    onAll("[data-action='toggle-status-metrics']", "click", () => { this._statusMetricsExpanded = !this._statusMetricsExpanded; this._render(); });
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  async _previewTemplateImport() {
    ...
    try {
      ...
      this._templateImportPreview = { valid: false, error: "json" };
      this._render();
      return;
    }
    this._templateImportPreview = await this.hass.callWS({ type: "maintenance_dashboard/preview_user_template_import", payload });
    this._render();
  },
  ...
  async _importUserTemplates() {
    ...
    const result = await this.hass.callWS({ type: "maintenance_dashboard/import_user_templates", payload, skip_duplicates: true });
    this._templateImportOpen = false;
    this._templateImportPayload = "";
    this._templateImportPreview = null;
    await this._load();
