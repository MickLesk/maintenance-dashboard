...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _taskCard(task) {
    ...
    const workflowActions = !completed ? `<div class="workflow-actions">
      ${canStart ? `<button class="ghost icon-only" title="${this._t("workflowStart")}" data-task-id="${task.id}" data-workflow-state="in_progress"><ha-icon icon="mdi:play"></ha-icon></button>` : `<button class="ghost icon-only" title="${this._t("workflowReady")}" data-task-id="${task.id}" data-workflow-state="ready"><ha-icon icon="mdi:rewind"></ha-icon></button>`}
      <button class="ghost icon-only" title="${canBlock ? this._t("workflowBlock") : this._t("workflowReady")}" data-task-id="${task.id}" data-workflow-state="${canBlock ? "blocked" : "ready"}"><ha-icon icon="${canBlock ? "mdi:pause-octagon-outline" : "mdi:play-pause"}"></ha-icon></button>
      <button class="ghost icon-only" title="${this._t("workflowResetAction")}" data-reset-task-progress="${task.id}"><ha-icon icon="mdi:restart"></ha-icon></button>
      ${task.schedule_mode !== "one_time" ? `<button class="ghost icon-only" title="${this._t("restartCycle")}" data-restart-task-cycle="${task.id}"><ha-icon icon="mdi:refresh"></ha-icon></button><button class="ghost icon-only" title="${this._t("skipCycle")}" data-skip-task-cycle="${task.id}"><ha-icon icon="mdi:skip-next"></ha-icon></button>` : ""}
    </div>` : "";
    return `<article class="task-card ${status}" data-task-card="${this._html(task.id)}" style="--task-accent:${this._html(accent)}">
      ...
      <div class="workflow-meta-row"><small>${this._t("runsCompleted")}: ${executionStats.completed || 0}</small><small>${this._t("runsSkipped")}: ${executionStats.skipped || 0}</small><small>${this._t("workflowResets")}: ${executionStats.resets || 0}</small></div>
      ...
      <footer class="actions">
        ...
        ${completed ? `<button class="primary" data-reactivate="${task.id}"><ha-icon icon="mdi:restore"></ha-icon>${this._t("reactivate")}</button>` : `${workflowActions}<div class="snooze-wrap"><button class="ghost icon-only" title="${this._t("snooze")}" data-snooze-menu="${task.id}"><ha-icon icon="mdi:clock-plus-outline"></ha-icon></button>${this._snoozeMenu === task.id ? `<div class="snooze-menu"><strong>${this._t("snoozeFor")}</strong>${options.map(days => `<button data-snooze-days="${task.id}:${days}">${days} ${this._t("days")}</button>`).join("")}</div>` : ""}</div>${snoozed ? `<button class="ghost" data-clear-snooze="${task.id}"><ha-icon icon="mdi:play-circle-outline"></ha-icon>${this._t("clearSnooze")}</button>` : ""}<button class="primary" data-done="${task.id}"><ha-icon icon="mdi:check"></ha-icon>${this._t("done")}</button>`}
      </footer>
    </article>`;
  }
});
    ...
    return `<section class="page-header page-header-compact"><div><h1>${this._t("templates")}</h1><p>${this._t("templateSelectHint")}</p></div></section>
      <section class="panel templates-workbench">
        ...
        <section class="template-filter-bar">
          ...
          <div class="template-secondary-filters">${resultMeta}<label><span>${this._t("seasonal")}</span><select id="templateSeason"><option value="all">${this._t("all")}</option>${["spring","summer","autumn","winter"].map(x => `<option value="${x}" ${this._templateSeason === x ? "selected" : ""}>${this._t(x)}</option>`).join("")}</select></label><label class="check compact-check"><input id="templateCommon" type="checkbox" ${this._templateOnlyCommon ? "checked" : ""}>${this._t("common")}</label></div>
        </section>
      </section>
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _settingsHtml() {
    ...
    const activeTab = ["general", "data", "workflow", "platforms", "manual"].includes(this._settingsTab) ? this._settingsTab : "general";
    return `
