...
const WORKFLOW_STATES = ["open", "in_progress", "blocked"];
const CURRENCIES = ["EUR", "USD", "GBP", "CHF", "SEK", "NOK", "DKK", "PLN", "CZK"];
// Must stay in sync with DASHBOARD_WIDGETS in const.py.
const DASHBOARD_WIDGET_KEYS = [
  "health", "open", "critical", "warning", "next", "due_today", "due_week", "due_month",
  "upcoming_week", "high_priority", "completed_this_year", "unavailable", "paused",
  "meter_tasks", "one_time", "notification_status",
];
...
const MODULE_KEYS = ["documents", "consumption", "inventory", "budget"];
// Must stay in sync with DOCUMENT_KINDS in const.py.
const DOCUMENT_KINDS = ["invoice", "proof", "warranty", "manual", "meter", "photo", "other"];
...
const I18N = Object.freeze({
  "de": {
    ...
    "applyQualityFix": "Qualitätsfix angewendet.",
    ...
    "areaFilter": "Bereich",
    "asset": "Gerät",
    "asset_expired": "Garantie oder Vertrag abgelaufen",
    "asset_expires_soon": "Garantie oder Vertrag läuft bald ab",
    "assetContract": "Betreuung",
    "assetContractHint": "Wer kümmert sich darum? Zum Beispiel der Schornsteinfeger oder die Heizungswartung.",
    "assetDevice": "Home-Assistant-Gerät",
    "assetDeviceHint": "Optional. Verknüpft den Eintrag mit einem Gerät aus Home Assistant.",
    "assetDeviceNoMatch": "Kein Gerät gefunden.",
    "assetDeviceSearch": "Gerät suchen…",
    "assetExpired": "abgelaufen",
    "assetExpiresIn": "läuft in {days} Tagen ab",
    "assetImport": "Anlagen aus Bereichen vorschlagen",
    "assetImportApply": "Ausgewählte anlegen",
    ...
    "attachmentStorageHint": "Ältere Anhänge werden entfernt, sobald die Grenze erreicht ist oder ihr Verlaufseintrag herausrotiert.",
    "attachmentTooLarge": "Anhang ist zu groß (max. 512 KB).",
    ...
    "budget": "Budget",
    "budgetAhead": "Schneller als geplant",
    "budgetExpected": "Erwartet bis heute",
    "budgetHint": "Ausgaben dieses Jahres gegen das geplante Budget.",
    "budgetOnTrack": "Im Plan",
    "budgetOver": "Budget überschritten",
    "budgetRemaining": "Rest",
    "budgetSettingsHint": "Jahresbudget je Kategorie. Leer lassen heißt: kein Budget.",
    "building": "Gebäude",
    ...
    "calendarEntityHint": "Native Kalender-Events für anstehende Fälligkeiten.",
    ...
    "completedWithMaterials": "Mit Material erfasst",
    "completionAttachments": "Fotos",
    ...
    "completionsByCategory": "Erledigungen nach Kategorie",
    "confirmBulk": "Mehrfachaktion wirklich ausführen?",
    "confirmBulkTitle": "Änderungen prüfen und bestätigen",
    "consumption_entity_unavailable": "Entität liefert keinen Wert",
    "consumption_meter_reset": "Zähler wurde zurückgesetzt",
    "consumption_module_disabled": "Modul ist ausgeschaltet",
    "consumption_no_statistics": "Noch keine Statistikdaten",
    "consumption_no_time_span": "Messpunkte ohne Zeitabstand",
    "consumption_not_a_meter_task": "Keine zählerbasierte Aufgabe",
    "consumption_not_enough_samples": "Zu wenige Messpunkte",
    "consumption_recorder_unavailable": "Recorder ist nicht verfügbar",
    "consumption_statistics_failed": "Statistik konnte nicht gelesen werden",
    "consumptionFixed": "Fester Grenzwert",
    "consumptionMode": "Zählerauswertung",
    "consumptionRate": "Verbrauchsrate (Prognose)",
    "consumptionRateValue": "Verbrauch",
    "consumptionRemaining": "Restlaufzeit",
    "consumptionUnavailable": "Keine Prognose möglich",
    "consumptionWindow": "Auswertungszeitraum (Tage)",
    ...
    "costTrackingHint": "Blendet Kostenfelder, Kostenauswertungen und den Kosten-Tab ein. Bereits erfasste Kosten bleiben erhalten.",
    ...
    "deleteAssetHint": "Gerät löschen? Die zugehörigen Wartungen bleiben erhalten und verlieren nur die Zuordnung.",
    "deleted": "Gelöscht",
    "deletedEvent": "Gelöscht",
    "deleteDocumentHint": "Diesen Eintrag wirklich löschen?",
    "deleteFilter": "Filter löschen",
    "deleteNote": "Notiz löschen",
    "deletePartHint": "Dieses Teil wirklich löschen?",
    "deleteRecord": "Datensatz löschen",
    "densityComfortable": "Komfortabel",
    "densityCompact": "Kompakt",
    "dependencyBlocked": "Blockiert durch {name}",
    "dependsOn": "Setzt voraus",
    "dependsOnHint": "Diese Aufgabe wird blockiert, solange die Voraussetzung überfällig ist.",
    "description": "Beschreibung",
    "deselectAll": "Auswahl leeren",
    "detailTabChecklist": "Checkliste",
    "detailTabCosts": "Kosten",
    "detailTabHistory": "Verlauf",
    "detailTabNotes": "Notizen",
    "detailTabOverview": "Übersicht",
    "diagnostics": "Diagnose",
    "digestTime": "Digest-Zeit",
    "disabled": "Deaktiviert",
    "documentChooseFile": "Datei auswählen",
    "documentFile": "Datei",
    "documentFileFailed": "Datei konnte nicht gelesen werden.",
    "documentFileHint": "Bild oder PDF. Kann auch leer bleiben, etwa für einen reinen Zählerstand.",
    "documentKind": "Art",
    "documentKind_invoice": "Rechnung",
    "documentKind_manual": "Anleitung",
    "documentKind_meter": "Zählerstand",
    "documentKind_other": "Sonstiges",
    "documentKind_photo": "Foto",
    "documentKind_proof": "Nachweis",
    "documentKind_warranty": "Garantie",
    "documentReading": "Zählerstand",
    "documentReadingUnit": "Einheit",
    "documents": "Ablage",
    "documentsEmpty": "Noch nichts abgelegt. Lege ein Gerät an oder häng direkt eine Datei an.",
    "documentsFolderEmpty": "Noch nichts abgelegt.",
    "documentsHint": "Geräte als Ordner. Rechnungen, Nachweise, Zählerstände und Anleitungen liegen darin.",
    "documentsUnfiled": "Ohne Zuordnung",
    "documentsUnfiledHint": "Alles, was zu keinem Gerät gehört.",
    "documentTitle": "Bezeichnung",
    "documentTitlePlaceholder": "z. B. Rechnung Heizungswartung 2026",
    "done": "Erledigt",
    "doneWithDetails": "Mit Details erledigen",
    ...
    "duplicateTaskWarning": "Mögliches Duplikat",
    "edit": "Bearbeiten",
    ...
    "exportTemplatesDone": "Vorlagen exportiert.",
    "favorite": "Favorit",
    "favorites": "Favoriten",
    ...
    "filterQualityIssues": "Qualitätsprobleme filtern",
    "filters": "Filter",
    "fixedDate": "Fixes Datum",
    "fixedDay": "Tag",
    "fixedMonth": "Monat",
    "focusNextTask": "Zur nächsten Aufgabe springen",
    "forecastHint": "Hochgerechnet aus den Intervallen der aktiven Aufgaben.",
    "forecastLoad": "Fällige Aufgaben je Monat",
    "forecastNotYearBound": "Die Prognose rechnet immer ab heute und folgt der Jahresauswahl nicht.",
    "forecastTotalCost": "Erwartete Kosten",
    "forecastTotalDue": "Fällig in 12 Monaten",
    ...
    "healthTrend": "Health-Verlauf",
    "healthTrendSince": "Aufzeichnung seit {date}",
    "healthTrendWaiting": "Der Verlauf beginnt heute. Ab morgen entsteht daraus eine Kurve.",
    "heating": "Heizung",
    ...
    "importTemplatePreview": "Import prüfen",
    "importTemplates": "Vorlagen importieren",
    "importTemplatesAction": "Vorlagen importieren",
    "importTemplatesHint": "JSON oder YAML mit templates-Array einfügen.",
    "importTemplatesPlaceholder": "{\"templates\":[{\"name\":\"…\"}]}",
    "importTemplatesYamlHint": "JSON oder YAML — einzelne Vorlage oder templates-Array.",
    ...
    "inventory": "Inventar",
    "inventoryEmpty": "Noch keine Ersatzteile erfasst.",
    "inventoryHint": "Ersatzteile und Verbrauchsmaterial mit Mindestbestand.",
    "inventoryParts": "Positionen",
    ...
    "later": "Später",
    "lessMetrics": "Weniger Kennzahlen",
    ...
    "module_budgetHint": "Jahresbudget je Kategorie, verglichen mit den echten Kosten.",
    "module_consumption": "Verbrauch",
    "module_consumptionHint": "Zählerverlauf und Restlaufzeit-Prognose.",
    "module_documents": "Ablage",
    "module_documentsHint": "Geräte, Garantien, Rechnungen, Nachweise und Fotos an einem Ort.",
    "module_inventory": "Inventar",
    "module_inventoryHint": "Ersatzteile, Bestände und Nachbestell-Warnungen.",
    "modules": "Module",
    "modulesHint": "Optionale Funktionsbereiche. Ausschalten blendet sie aus, gelöscht wird nichts.",
    "monthly": "Monatlich",
    ...
    "moreMetrics": "Mehr Kennzahlen",
    "name": "Name",
    "nameMissing": "Name ist erforderlich",
    "nativePlatforms": "Native Home-Assistant-Plattformen",
    "nativePlatformsHint": "To-do- und Kalender-Plattformen für Automatisierungen und mobile Ansichten.",
    "nativePlatformsTitle": "Home Assistant Sync",
    "navigation": "Navigation",
    "newAsset": "Neues Gerät",
    ...
    "noDependency": "Keine Voraussetzung",
    ...
    "notifications_disabled": "Benachrichtigungen sind ausgeschaltet",
    "notificationsDisabledWarning": "Ein Dienst ist ausgewählt, aber Benachrichtigungen sind ausgeschaltet. Es wird nichts gesendet.",
    ...
    "notifyServiceHint": "Dienst der Home-Assistant-App, an den Push-Benachrichtigungen gehen.",
    "notifyServiceInherit": "Globale Einstellung verwenden",
    "notifyServiceMissing": "Kein notify.*-Dienst gefunden. Ohne einen solchen Dienst landen Benachrichtigungen nur als Hinweis in Home Assistant, nicht als Push auf dem Handy.",
    "notifyServiceNone": "Kein Dienst ausgewählt",
    "notifyServiceUnknown": "nicht gefunden",
    ...
    "numberFormatAuto": "Automatisch (Sprache von Home Assistant)",
    "numberFormatComma": "1.234,56",
    "numberFormatDot": "1,234.56",
    ...
    "onboardingModulesHint": "Zusatzbereiche. Alles hier lässt sich später in den Einstellungen ändern.",
    "onboardingNotifications": "Benachrichtigungen einrichten",
    "onboardingNotificationsHint": "Optional. Ohne einen notify.*-Dienst erscheinen Hinweise nur in Home Assistant, nicht als Push auf dem Handy.",
    ...
    "onTimeRate": "Pünktlich erledigt",
    "open": "Offen",
    "openBreakdown": "Offene Aufgaben",
    "openDetails": "Details öffnen",
    "openHistory": "Verlauf öffnen",
    "openReport": "Jahresbericht öffnen",
    "openReportHint": "Druckbarer HTML-Bericht mit Fotos, Kosten, Material und Beteiligten.",
    "overdue": "Überfällig",
    ...
    "partLocationPlaceholder": "z. B. Regal Keller",
    "partMinimum": "Mindestbestand",
    "partName": "Bezeichnung",
    "partNumber": "Teilenummer",
    "partPurchaseUrl": "Nachbestellen (Link)",
    "partRestock": "Zubuchen",
    "partStock": "Bestand",
    "partSupplier": "Lieferant",
    "partTake": "Entnehmen",
    "partUnit": "Einheit",
    "partUnitPrice": "Stückpreis",
    "pausedUntil": "Pausiert bis",
    "pendingRepairs": "Offene Reparaturen",
    "performedBy": "Ausgeführt von",
    "phase_after": "Nachher",
    "phase_before": "Vorher",
    "phase_single": "Ohne Zuordnung",
    "photoGallery": "Fotos",
    ...
    "procedureDuration": "ca. {minutes} Min.",
    "processNotificationsNow": "Regeln jetzt prüfen",
    "progress": "Fortschritt",
    "qualityCheck": "Qualitätscheck",
    "qualityFix_category": "Kategorie setzen",
    "qualityFix_description": "Beschreibung ergänzen",
    "qualityFix_interval": "Intervall kürzen",
    "qualityFix_tags": "Tag hinzufügen",
    "qualityFixDescriptionDefault": "Wartungshinweise ergänzen.",
    "qualityIssue_category": "Kategorie fehlt",
    "qualityIssue_description": "Beschreibung fehlt",
    "qualityIssue_interval": "Intervall sehr lang",
    "qualityIssue_tags": "Keine Tags",
    "qualityIssuesTitle": "Qualitätscheck",
    ...
    "ratedCompletions": "Bewertete Erledigungen",
    "reactivate": "Reaktivieren",
    "reactivatedEvent": "Reaktiviert",
    "recommended": "Empfohlen",
    "recommendedOnly": "Nur empfohlen",
    "records": "Datensätze",
    "reliabilityHint": "Vergleicht den Erledigungszeitpunkt mit dem Fälligkeitsdatum.",
    ...
    "runLabel": "Durchlauf",
    "runsCompletedTotal": "Abgeschlossen",
    "runsHint": "Zählt abgeschlossene und übersprungene Durchläufe je Aufgabe.",
    "runsLifetime": "Gesamt seit Beginn",
    "runsResetTotal": "Zurückgesetzt",
    "runsRestartedTotal": "Neu gestartet",
    "runsSkippedTotal": "Übersprungen",
    "runsYearHint": "Gezählt aus dem Verlauf des gewählten Jahres. Ältere Einträge können durch die Verlaufsbegrenzung fehlen.",
    "safety": "Sicherheit",
    ...
    "settingsSearchEmpty": "Keine Einstellung gefunden",
    "settingsSection": "Einstellungen",
    "shortcutCloseOverlay": "Dialog oder Detail schließen",
    ...
    "similarTaskExists": "Ähnliche Aufgabe existiert bereits",
    "skip": "Überspringen",
    "skipCycle": "Zyklus überspringen",
    "skipRate": "Übersprungen-Quote",
    "snooze": "Pausieren",
    ...
    "statisticsHint": "Kosten, Material und Erledigungen für das laufende Jahr.",
    "statisticsNeedsHistory": "Sobald Aufgaben erledigt werden, erscheinen hier Auswertungen.",
    "statisticsReliability": "Zuverlässigkeit",
    "statisticsRuns": "Durchläufe",
    "statisticsYear": "Jahr",
    "status": "Status",
    "stepDuration": "Dauer (Min.)",
    "stepHint": "Anleitung",
    "stepHintPlaceholder": "Was ist bei diesem Schritt zu tun?",
    "stepMinutes": "{minutes} Min.",
    "stepNoPart": "Kein Teil",
    "stepPart": "Ersatzteil",
    "stepPartShort": "Bestand reicht nicht",
    "stepQuantity": "Menge",
    "summaryBulk": "Sammelaktion wurde ausgeführt.",
    "summaryCompleted": "Aufgabe wurde erledigt.",
    "summaryCreated": "Aufgabe wurde angelegt.",
    "summaryDeleted": "Aufgabe wurde gelöscht.",
    "summaryImported": "Daten wurden importiert.",
    "summaryReactivated": "Einmalige Aufgabe wurde reaktiviert.",
    "summaryRestored": "Aufgabe wurde wiederhergestellt.",
    "summarySnoozeCleared": "Pause wurde aufgehoben.",
    "summarySnoozed": "Aufgabe wurde pausiert.",
    "summaryUndoCompleted": "Erledigung wurde zurückgenommen.",
    "summaryUpdated": "Aufgabe wurde bearbeitet.",
    "summer": "Sommer",
    "tagFilter": "Tag",
    ...
    "taskType": "Typ",
    "templateAppliedFrom": "Vorlage: {name}",
    "templateAppliedHint": "Alle Felder wurden aus der Vorlage übernommen.",
    ...
    "templateSearchPlaceholder": "Vorlage suchen…",
    ...
    "todayRange": "Heute",
    "todoEntityHint": "Native To-do-Liste mit offenen Wartungsaufgaben.",
    "todoIncludeDisabled": "Deaktivierte Aufgaben in To-do anzeigen",
    "todoPlatform": "To-do-Plattform",
    "topCostTasks": "Teuerste Aufgaben",
    "topMaterials": "Häufigste Materialien",
    ...
    "warnings": "Warnungen",
    "wasAheadBy": "War {value} vor Fälligkeit",
    "wasOverdueBy": "War {value} überfällig",
    "water": "Wasser",
    "weekRange": "Diese Woche",
    "weeks": "Wochen",
    "widget_completed_this_year": "Dieses Jahr erledigt",
    "widget_critical": "Kritisch",
    "widget_due_month": "In 30 Tagen fällig",
    "widget_due_today": "Heute fällig",
    "widget_due_week": "In 7 Tagen fällig",
    "widget_health": "Health-Score",
    "widget_high_priority": "Hohe Priorität",
    "widget_meter_tasks": "Zählerbasiert",
    "widget_next": "Nächste Aufgabe",
    "widget_notification_status": "Benachrichtigungen",
    "widget_one_time": "Einmalig",
    "widget_open": "Offen",
    "widget_paused": "Pausiert",
    "widget_unavailable": "Nicht auswertbar",
    "widget_upcoming_week": "Kommende Woche",
    "widget_warning": "Warnung",
    "widgets": "Kennzahlen",
    "widgetsHint": "Welche Kennzahlen in der Statuszeile über dem Dashboard erscheinen.",
    "winter": "Winter",
    "withoutEntity": "Ohne Entity",
    "wizardBack": "Zurück",
    "wizardContinueWithTemplate": "Weiter mit Vorlage",
    ...
    "wizardStartHintEmpty": "Gib Name, Kategorie und optional Bereich an – ohne Vorlage.",
    "wizardStartHintFilled": "Passe die vorgefüllten Angaben nach Bedarf an.",
    "wizardStepStart": "Start",
    "wizardTemplatePickHint": "Suche und wähle eine Vorlage. Die Felder werden danach automatisch ausgefüllt.",
    "workflow": "Workflow",
    ...
    "workflowSet_open": "Aufgabe ist wieder offen.",
    "workflowSetOpen": "Auf offen setzen",
  ...
  "en": {
    ...
    "applyQualityFix": "Quality fix applied.",
    ...
    "areaFilter": "Area",
    "asset": "Device",
    "asset_expired": "Warranty or contract expired",
    "asset_expires_soon": "Warranty or contract expires soon",
    "assetContract": "Care",
    "assetContractHint": "Who takes care of it? For example the chimney sweep or the boiler service.",
    "assetDevice": "Home Assistant device",
    "assetDeviceHint": "Optional. Links the entry to a Home Assistant device.",
    "assetDeviceNoMatch": "No device found.",
    "assetDeviceSearch": "Search device…",
    "assetExpired": "expired",
    "assetExpiresIn": "expires in {days} days",
    "assetImport": "Suggest assets from areas",
    "assetImportApply": "Create selected",
    ...
    "attachmentStorageHint": "Older attachments are removed once the limit is reached or their history entry rotates out.",
    "attachmentTooLarge": "Attachment is too large (max. 512 KB).",
    ...
    "brandName": "Maintenance Dashboard",
    "budget": "Budget",
    "budgetAhead": "Ahead of pace",
    "budgetExpected": "Expected by now",
    "budgetHint": "This year's spend against the planned budget.",
    "budgetOnTrack": "On track",
    "budgetOver": "Over budget",
    "budgetRemaining": "Remaining",
    "budgetSettingsHint": "Yearly budget per category. Leave empty for no budget.",
    "building": "Building",
    ...
    "calendarEntityHint": "Native calendar events for upcoming due dates.",
    ...
    "completedWithMaterials": "Recorded with material",
    "completionAttachments": "Photos",
    "completionCost": "Cost",
    ...
    "completionsByCategory": "Completions by category",
    "confirmBulk": "Run this bulk action?",
    "confirmBulkTitle": "Review and confirm changes",
    "consumption_entity_unavailable": "Entity provides no value",
    "consumption_meter_reset": "Meter was reset",
    "consumption_module_disabled": "Module is switched off",
    "consumption_no_statistics": "No statistics recorded yet",
    "consumption_no_time_span": "Samples without a time span",
    "consumption_not_a_meter_task": "Not a meter based task",
    "consumption_not_enough_samples": "Not enough samples",
    "consumption_recorder_unavailable": "Recorder is unavailable",
    "consumption_statistics_failed": "Statistics could not be read",
    "consumptionFixed": "Fixed limit",
    "consumptionMode": "Meter evaluation",
    "consumptionRate": "Consumption rate (prediction)",
    "consumptionRateValue": "Consumption",
    "consumptionRemaining": "Remaining life",
    "consumptionUnavailable": "No prediction available",
    "consumptionWindow": "Evaluation window (days)",
    ...
    "costTrackingHint": "Shows cost fields, cost reports and the cost tab. Costs already recorded are kept either way.",
    ...
    "delete": "Delete",
    "deleteAsset": "Delete device",
    "deleteAssetHint": "Delete this device? Its maintenance tasks are kept and only lose the link.",
    "deleted": "Deleted",
    "deletedEvent": "Deleted",
    "deleteDocumentHint": "Really delete this entry?",
    "deleteFilter": "Delete filter",
    "deleteNote": "Delete note",
    "deletePartHint": "Really delete this part?",
    "deleteRecord": "Delete record",
    "densityComfortable": "Comfortable",
    "densityCompact": "Compact",
    "dependencyBlocked": "Blocked by {name}",
    "dependsOn": "Depends on",
    "dependsOnHint": "This task is blocked while its prerequisite is overdue.",
    "description": "Description",
    "deselectAll": "Clear selection",
    "detailTabChecklist": "Checklist",
    ...
    "documentFile": "File",
    "documentFileFailed": "The file could not be read.",
    "documentFileHint": "Image or PDF. May be left empty, for instance for a plain meter reading.",
    "documentKind": "Type",
    "documentKind_invoice": "Invoice",
    "documentKind_manual": "Manual",
    "documentKind_meter": "Meter reading",
    "documentKind_other": "Other",
    "documentKind_photo": "Photo",
    "documentKind_proof": "Proof",
    "documentKind_warranty": "Warranty",
    "documentReading": "Reading",
    "documentReadingUnit": "Unit",
    "documents": "Files",
    "documentsEmpty": "Nothing filed yet. Add a device, or attach a file directly.",
    "documentsFolderEmpty": "Nothing filed yet.",
    "documentsHint": "Devices as folders. Invoices, proofs, meter readings and manuals live inside them.",
    "documentsUnfiled": "Unfiled",
    "documentsUnfiledHint": "Everything that belongs to no device.",
    "documentTitle": "Name",
    "documentTitlePlaceholder": "e.g. Boiler service invoice 2026",
    "done": "Done",
    "doneWithDetails": "Complete with details",
    ...
    "duplicateSkip": "Skip",
    "duplicateTaskWarning": "Possible duplicate",
    "edit": "Edit",
    "editNote": "Edit note",
    ...
    "exportTemplatesDone": "Templates exported.",
    "favorite": "Favorite",
    "favorites": "Favorites",
    ...
    "filterQualityIssues": "Filter quality issues",
    "filters": "Filters",
    "fixedDate": "Fixed date",
    "fixedDay": "Day",
    "fixedMonth": "Month",
    "focusNextTask": "Jump to next task",
    "forecastHint": "Projected from the intervals of the active tasks.",
    "forecastLoad": "Tasks due per month",
    "forecastNotYearBound": "The forecast always starts today and does not follow the year selector.",
    "forecastTotalCost": "Expected cost",
    "forecastTotalDue": "Due within 12 months",
    "frontendLabel": "Frontend",
    "full": "Full",
    "garage": "Garage",
    "garden": "Garden",
    ...
    "healthTrendSince": "Recording since {date}",
    "healthTrendWaiting": "Recording starts today. A line appears from tomorrow on.",
    "heating": "Heating",
    ...
    "importTemplatePreview": "Preview import",
    "importTemplates": "Import templates",
    "importTemplatesAction": "Import templates",
    "importTemplatesHint": "Paste JSON or YAML with a templates array.",
    "importTemplatesPlaceholder": "{\"templates\":[{\"name\":\"…\"}]}",
    "importTemplatesYamlHint": "JSON or YAML — single template or templates array.",
    ...
    "inventory": "Inventory",
    "inventoryEmpty": "No spare parts recorded yet.",
    "inventoryHint": "Spare parts and consumables with a minimum stock level.",
    "inventoryParts": "Items",
    ...
    "mobileMore": "More",
    "module_budget": "Budget",
    "module_budgetHint": "Yearly budget per category, compared against real costs.",
    "module_consumption": "Consumption",
    "module_consumptionHint": "Meter history and remaining-life prediction.",
    "module_documents": "Files",
    "module_documentsHint": "Devices, warranties, invoices, proofs and photos in one place.",
    "module_inventory": "Inventory",
    "module_inventoryHint": "Spare parts, stock levels and restock alerts.",
    "modules": "Modules",
    "modulesHint": "Optional feature areas. Switching one off hides it; nothing is deleted.",
    "monthly": "Monthly",
    ...
    "nativePlatformsHint": "To-do and calendar platforms for automations and mobile views.",
    "nativePlatformsTitle": "Home Assistant sync",
    "navigation": "Navigation",
    "newAsset": "New device",
    "newEntry": "New entry",
    ...
    "notifications_disabled": "Notifications are switched off",
    "notificationsDisabledWarning": "A service is selected but notifications are switched off. Nothing will be sent.",
    ...
    "notifyService": "Notify service",
    "notifyServiceHint": "The Home Assistant app service that receives push notifications.",
    "notifyServiceInherit": "Use the global setting",
    "notifyServiceMissing": "No notify.* service found. Without one, notifications only appear inside Home Assistant instead of as a push message.",
    "notifyServiceNone": "No service selected",
    "notifyServiceUnknown": "not found",
    ...
    "numberFormatAuto": "Automatic (Home Assistant language)",
    "numberFormatComma": "1.234,56",
    "numberFormatDot": "1,234.56",
    ...
    "onboardingModulesHint": "Extra areas. All of this can be changed later in the settings.",
    "onboardingNotifications": "Set up notifications",
    "onboardingNotificationsHint": "Optional. Without a notify.* service, reminders only appear inside Home Assistant instead of as a push message.",
    ...
    "openReport": "Open yearly report",
    "openReportHint": "Printable HTML report with photos, costs, materials and people.",
    "overdue": "Overdue",
    ...
    "partLocationPlaceholder": "e.g. basement shelf",
    "partMinimum": "Minimum stock",
    "partName": "Name",
    "partNumber": "Part number",
    "partPurchaseUrl": "Reorder link",
    "partRestock": "Book in",
    "partStock": "Stock",
    ...
    "procedureDuration": "approx. {minutes} min",
    "processNotificationsNow": "Evaluate rules now",
    "progress": "Progress",
    "qualityCheck": "Quality check",
    "qualityFix_category": "Set category",
    "qualityFix_description": "Add description",
    "qualityFix_interval": "Shorten interval",
    "qualityFix_tags": "Add tag",
    "qualityFixDescriptionDefault": "Add maintenance notes.",
    "qualityIssue_category": "Missing category",
    "qualityIssue_description": "Missing description",
    "qualityIssue_interval": "Very long interval",
    "qualityIssue_tags": "No tags",
    "qualityIssuesTitle": "Quality check",
    ...
    "ratedCompletions": "Rated completions",
    "reactivate": "Reactivate",
    "reactivatedEvent": "Reactivated",
    "recommended": "Recommended",
    "recommendedOnly": "Recommended only",
    "records": "records",
    "reliabilityHint": "Compares when a task was completed against when it was due.",
    "remaining": "remaining",
    ...
    "runLabel": "Run",
    "runsCompletedTotal": "Completed",
    "runsHint": "Counts completed and skipped runs per task.",
    "runsLifetime": "Lifetime total",
    "runsResetTotal": "Reset",
    "runsRestartedTotal": "Restarted",
    "runsSkippedTotal": "Skipped",
    "runsYearHint": "Counted from the history of the selected year. Older entries may be missing due to history retention.",
    "safety": "Safety",
    ...
    "settingsSearchEmpty": "No setting matches",
    "settingsSection": "Settings",
    "shortcutCloseOverlay": "Close dialog or detail",
    ...
    "similarTaskExists": "A similar task already exists",
    "skip": "Skip",
    ...
    "statisticsHint": "Costs, materials and completions for the current year.",
    "statisticsNeedsHistory": "Reports appear here once tasks have been completed.",
    "statisticsReliability": "Reliability",
    "statisticsRuns": "Runs",
    "statisticsYear": "Year",
    "status": "Status",
    "stepDuration": "Duration (min)",
    "stepHint": "Instruction",
    "stepHintPlaceholder": "What needs to be done in this step?",
    "stepMinutes": "{minutes} min",
    "stepNoPart": "No part",
    "stepPart": "Spare part",
    "stepPartShort": "not enough in stock",
    "stepQuantity": "Quantity",
    "summaryBulk": "Bulk action was applied.",
    "summaryCompleted": "Task was completed.",
    "summaryCreated": "Task was created.",
    "summaryDeleted": "Task was deleted.",
    "summaryImported": "Data was imported.",
    "summaryReactivated": "One-time task was reactivated.",
    "summaryRestored": "Task was restored.",
    "summarySnoozeCleared": "Snooze was cleared.",
    "summarySnoozed": "Task was snoozed.",
    "summaryUndoCompleted": "Completion was undone.",
    "summaryUpdated": "Task was updated.",
    "summer": "Summer",
    "tagFilter": "Tag",
    ...
    "templateAppliedHint": "All fields were taken from the template.",
    ...
    "todayRange": "Today",
    "todoEntityHint": "Native to-do list with open maintenance tasks.",
    "todoIncludeDisabled": "Show disabled tasks in To-do",
    "todoPlatform": "To-do platform",
    "topCostTasks": "Top cost tasks",
    ...
    "warnings": "Warnings",
    "wasAheadBy": "Was {value} ahead of schedule",
    "wasOverdueBy": "Was {value} overdue",
    "water": "Water",
    "weekRange": "This week",
    "weeks": "Weeks",
    "widget_completed_this_year": "Completed this year",
    "widget_critical": "Critical",
    "widget_due_month": "Due within 30 days",
    "widget_due_today": "Due today",
    "widget_due_week": "Due within 7 days",
    "widget_health": "Health score",
    "widget_high_priority": "High priority",
    "widget_meter_tasks": "Meter based",
    "widget_next": "Next task",
    "widget_notification_status": "Notifications",
    "widget_one_time": "One-time",
    "widget_open": "Open",
    "widget_paused": "Paused",
    "widget_unavailable": "Unavailable",
    "widget_upcoming_week": "Upcoming week",
    "widget_warning": "Warning",
    "widgets": "Metrics",
    "widgetsHint": "Which metrics appear in the status line above the dashboard.",
    "winter": "Winter",
    "withoutEntity": "Without entity",
    "wizardBack": "Back",
    "wizardContinueWithTemplate": "Continue with template",
    ...
    "wizardStartHintEmpty": "Enter name, category and optional area – without a template.",
    "wizardStartHintFilled": "Adjust the pre-filled values as needed.",
    "wizardStepStart": "Start",
    "wizardTemplatePickHint": "Search and pick a template. Fields will be filled in automatically.",
    "workflow": "Workflow",
 ...
 * @property {"open"|"in_progress"|"blocked"} state
 * @property {"completed"|"skipped"|"canceled"|null} outcome
 ...
 * @property {"open"|"in_progress"|"blocked"} [workflow_state]
...
// @ts-nocheck
// Main custom element shell and lifecycle. Feature/rendering methods are attached
// by split modules, so TypeScript cannot resolve them here. tests/test_panel_bundle.py
// resolves every call against its definition in the built bundle instead.
class MaintenanceDashboardPanel extends HTMLElement {
  constructor() {
    ...
    this._assetDraft = null;
    this._assetProposals = null;
    this._partDraft = null;
    this._documentDraft = null;
  ...
  async _load() {
    ...
    try {
      ...
      if (!this._statisticsYear) this._statisticsYear = Number((this._state?.statistics_years || [])[0] || new Date().getFullYear());
      // Keep the cached statistics in step with the data they describe. A failure
      // here must not look like the whole state failed to load.
      if (this._statisticsData && !this._statisticsData.error && this.hass?.callWS) {
        try {
          this._statisticsData = await this.hass.callWS({ type: "maintenance_dashboard/get_statistics", year: this._statisticsYear });
        } catch {
          this._statisticsData = null;
        }
      }
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _hero() {
    return `<header class="hero top-app-bar"><div class="hero-brand compact-brand"><strong>${this._t("brandName")}</strong><small class="brand-version" title="${this._t("version")}">v${this._html(VERSION)}</small></div><nav class="hero-actions" aria-label="${this._t("navigation")}">${this._nav("dashboard", "mdi:view-dashboard", this._t("dashboard"))}${this._documentsEnabled() ? this._nav("documents", "mdi:folder-outline", this._t("documents")) : ""}${this._inventoryEnabled() ? this._nav("inventory", "mdi:package-variant-closed", this._t("inventory")) : ""}${this._nav("statistics", "mdi:chart-box-outline", this._t("statistics"))}${this._nav("history", "mdi:history", this._t("history"))}${this._nav("settings", "mdi:cog", this._t("settings"))}</nav></header>`;
  },
  ...
  _viewHtml() {
    ...
    if (this._view === "documents") return this._documentsEnabled() ? this._documentsHtml() : this._dashboardHtml();
    if (this._view === "inventory") return this._inventoryEnabled() ? this._inventoryHtml() : this._dashboardHtml();
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _dashboardHtml() {
    ...
    return `
      ...
      ${this._dashboardActiveFilterChipsHtml()}
  ...
  // Renders the metrics configured under Settings, General. The first three are
  // always visible, the rest sit behind the expand toggle.
  _dashboardWidgetsHtml() {
    const configured = this._state?.settings?.dashboard?.widgets || [];
    const chips = configured.map(key => this._widgetChip(key)).filter(Boolean);
    const quality = `<button class="status-metric status-metric-action" data-action="open-quality-dialog"><ha-icon icon="mdi:clipboard-search-outline"></ha-icon><b>${this._qualityIssues().length}</b> ${this._t("qualityCheck")}</button>`;
    const primary = chips.slice(0, 3);
    const secondary = [...chips.slice(3), quality];
  ...
  _widgetChip(key) {
    ...
    const metric = (icon, value, label, extra = "", cls = "") =>
      `<span class="status-metric ${cls}"><ha-icon icon="${icon}"></ha-icon><b>${value}</b> ${label}${extra}</span>`;
    switch (key) {
      case "health": {
        const health = Number(s.health ?? 100);
        return `<strong class="status-health ${health < 70 ? "warning" : ""}"><ha-icon icon="mdi:heart-pulse"></ha-icon>${health}% ${this._t("health")}</strong>`;
      }
      case "open":
        return `<span class="status-metric metric-popover"><ha-icon icon="mdi:circle-outline"></ha-icon><b>${s.open ?? s.active ?? 0}</b> ${this._t("active")}${this._openBreakdownHtml()}</span>`;
      case "critical":
        return metric("mdi:alert-circle-outline", s.critical ?? 0, this._t("critical"), "", s.critical ? "critical" : "");
      case "warning":
        return metric("mdi:alert-outline", s.warning ?? 0, this._t("warning"), "", s.warning ? "warning" : "");
      case "next": {
        const next = s.next_task || {};
        if (!next.name) return "";
        return `<span class="status-metric"><ha-icon icon="mdi:arrow-right-circle-outline"></ha-icon>${this._html(next.name)}</span>`;
      }
      case "due_today":
        return metric("mdi:calendar-today-outline", this._countDueByFilter("today"), this._t("todayFocus"));
      case "due_week":
      case "upcoming_week":
        return metric("mdi:calendar-week-outline", this._countDueWithinDays(7), this._t("dueNext7"));
      case "due_month":
        return metric("mdi:calendar-month-outline", this._countDueWithinDays(30), this._t("dueNext30"));
      case "high_priority":
        return metric("mdi:flag-variant", this._countHighPriority(), this._t("highPriority"));
      case "completed_this_year":
        return metric("mdi:check-circle-outline", s.completed_this_year ?? 0, this._t("completedThisYear"));
      case "unavailable":
        return Number(s.unavailable ?? 0)
          ? `<span class="status-metric" title="${this._t("unavailableHelp")}"><ha-icon icon="mdi:cloud-question-outline"></ha-icon><b>${s.unavailable}</b> ${this._t("unavailable")}</span>`
          : "";
      case "paused":
        return metric("mdi:pause-circle-outline", s.snoozed ?? 0, this._t("snoozed"));
      case "meter_tasks":
        return metric("mdi:gauge", this._countByTaskType("meter"), this._t("meter"));
      case "one_time":
        return metric("mdi:calendar-check-outline", this._countBySchedule("one_time"), this._t("oneTime"));
      case "notification_status": {
        const enabled = this._state?.settings?.notifications?.enabled !== false;
        return `<span class="status-metric"><ha-icon icon="${enabled ? "mdi:bell-outline" : "mdi:bell-off-outline"}"></ha-icon>${this._t(enabled ? "enabled" : "disabled")}</span>`;
      }
      default:
        return "";
    }
  ...
  _openBreakdownHtml() {
    ...
    const workflow = [
      ...
      ["open", "workflow_open", tasks.filter(task => task.workflow_state === "open").length],
    ].filter(([, , count]) => count);
    const rows = [
  ...
  _dashboardActiveFilterChipsHtml() {
    const chips = this._activeDashboardFilters();
    if (!chips.length) return "";
    return `<section class="dashboard-filter-chips"><div class="active-filter-chips">${chips.map(chip => `<button class="active-filter-chip" data-clear-dashboard-filter="${this._html(chip.key)}">${this._html(chip.label)}<ha-icon icon="mdi:close"></ha-icon></button>`).join("")}<button class="ghost small" data-action="reset-filters"><ha-icon icon="mdi:filter-remove-outline"></ha-icon>${this._t("clear")}</button></div></section>`;
  },
  ...
  _bulkValueInputHtml() {
    ...
    if (this._bulkAction === "workflow") return `<label class="bulk-toolbar-field"><span>${this._t("workflowState")}</span><select id="bulkValue">${WORKFLOW_STATES.map(state => `<option value="${state}" ${String(this._bulkValue || "open") === state ? "selected" : ""}>${this._workflowStateLabel(state)}</option>`).join("")}</select></label>`;
  ...
  _compactTaskRow(task) {
    ...
    const workflowState = task.workflow_state || "open";
    ...
    const actions = mobile
      ...
      : `<button class="icon" data-edit="${task.id}"><ha-icon icon="mdi:pencil"></ha-icon></button>${completed ? `<button class="icon" data-reactivate="${task.id}"><ha-icon icon="mdi:restore"></ha-icon></button>` : `<button class="icon" data-task-id="${task.id}" data-workflow-state="${workflowState === "in_progress" ? "open" : "in_progress"}"><ha-icon icon="${workflowState === "in_progress" ? "mdi:rewind" : "mdi:play"}"></ha-icon></button><button class="primary small" data-done="${task.id}"><ha-icon icon="mdi:check"></ha-icon></button>`}`;
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _taskDetailSheetHtml() {
    ...
    const tabs = [
      ...
      ...(this._documentsEnabled() ? [["photos", "photoGallery", "mdi:image-multiple-outline"]] : []),
      ...(this._costTrackingEnabled() ? [["costs", "detailTabCosts", "mdi:cash-multiple"]] : []),
    ];
    ...
    const costsTab = `<section class="detail-section"><div class="detail-metrics"><div><span>${this._t("yearlyCosts")}</span><strong>${totalCost}</strong></div></div><div class="detail-timeline">${history.filter(event => event.type === "completed").map(event => {
      ...
      return `<article><ha-icon icon="mdi:cash"></ha-icon><div><strong>${this._html(this._money(completion.cost, completion.currency))}</strong><p>${this._html(completion.material || completion.note || "")}</p><small>${this._datetime(event.created_at)}</small></div></article>`;
    ...
    const body = {
      ...
      notes: notesTab,
      history: historyTab,
      ...(this._documentsEnabled() ? { photos: this._taskPhotosHtml(task.id) } : {}),
      ...(this._costTrackingEnabled() ? { costs: costsTab } : {}),
    }[tab] || overview;
  ...
  _taskPhotosHtml(taskId) {
    const grouped = this._taskMedia(taskId);
    const section = (phase) => {
      const items = grouped[phase] || [];
      if (!items.length) return "";
      return `<h3>${this._t(`phase_${phase}`)}</h3><div class="photo-grid">${items.map(item => `<a class="photo-tile" href="${this._mediaUrl(item.id)}" target="_blank" rel="noopener"><img loading="lazy" src="${this._mediaUrl(item.id)}" alt="${this._html(item.filename)}"></a>`).join("")}</div>`;
    };
    const body = ["before", "after", "single"].map(section).join("");
    return `<section class="detail-section">${body || `<p class="section-hint">${this._t("noDataYet")}</p>`}</section>`;
  },
  ...
  _taskYearCost(taskId) {
    ...
    return total ? this._money(total) : "—";
  },
});
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _taskCard(task) {
    ...
    const primaryWorkflow = canStart
      ...
      : `<button class="ghost" title="${this._t("workflowSetOpen")}" data-task-id="${task.id}" data-workflow-state="open"><ha-icon icon="mdi:pause"></ha-icon>${this._t("workflowOpenShort")}</button>`;
    const workflowMenu = !completed ? `<div class="workflow-menu-wrap"><button class="ghost icon-only" title="${this._t("moreActions")}" data-workflow-menu="${task.id}"><ha-icon icon="mdi:dots-vertical"></ha-icon></button>${this._workflowMenu === task.id ? `<div class="workflow-menu"><button data-task-id="${task.id}" data-workflow-state="${canBlock ? "blocked" : "open"}"><ha-icon icon="${canBlock ? "mdi:pause-octagon-outline" : "mdi:play-pause"}"></ha-icon>${canBlock ? this._t("workflowBlock") : this._t("workflowSetOpen")}</button><button data-done-details="${task.id}"><ha-icon icon="mdi:clipboard-edit-outline"></ha-icon>${this._t("doneWithDetails")}</button><button data-reset-task-progress="${task.id}"><ha-icon icon="mdi:restart"></ha-icon>${this._t("workflowResetAction")}</button>${task.schedule_mode !== "one_time" ? `<button data-restart-task-cycle="${task.id}"><ha-icon icon="mdi:refresh"></ha-icon>${this._t("restartCycle")}</button><button data-skip-task-cycle="${task.id}"><ha-icon icon="mdi:skip-next"></ha-icon>${this._t("skipCycle")}</button>` : ""}</div>` : ""}</div>` : "";
    const mobile = this._isMobileViewport();
...
// ---- frontend/src/views/documents-view.ts ----
...
// ---- frontend/src/views/inventory-view.ts ----
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  _partCard(part) {
    ...
    return `<article class="panel asset-card">
      ...
      <div class="meta-grid">
        ...
        ${part.location ? `<div><span>${this._t("partLocation")}</span><strong>${this._html(part.location)}</strong></div>` : ""}
      ...
      <div class="part-actions">
        ...
        ${part.purchase_url ? `<a class="ghost small" href="${this._html(part.purchase_url)}" target="_blank" rel="noopener noreferrer"><ha-icon icon="mdi:open-in-new"></ha-icon>${this._t("partBuy")}</a>` : ""}
      </div>
    </article>`;
  },
  ...
  _partDialogHtml() {
    ...
    if (!draft) return "";
    const assets = this._documentsEnabled() ? this._assetList() : [];
    const field = (key, label, type = "text", extra = "") =>
      `<label class="field"><span>${label === "partUnitPrice" ? `${this._t(label)} (${this._defaultCurrency()})` : this._t(label)}</span><input data-part-field="${key}" type="${type}" ${extra} value="${this._html(draft[key] ?? "")}"></label>`;
    return `<div class="dialog-backdrop"><section class="dialog"><header><div class="dialog-title-block"><h2>${draft.id ? this._t("edit") : this._t("newPart")}</h2></div><button class="icon" data-action="close-part-dialog"><ha-icon icon="mdi:close"></ha-icon></button></header><div class="dialog-body">
      <section class="dialog-section"><div class="form-grid">
        ...
        ${field("location", "partLocation", "text", `placeholder="${this._t("partLocationPlaceholder")}"`)}
        ${field("unit", "partUnit")}
        ...
        ${field("purchase_url", "partPurchaseUrl", "url", 'placeholder="https://"')}
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  _mobileActionSheetHtml() {
    ...
    const actions = completed
      ...
      : `<button class="primary" data-done="${task.id}"><ha-icon icon="mdi:check"></ha-icon>${this._t("done")}</button><button class="ghost" data-task-id="${task.id}" data-workflow-state="in_progress"><ha-icon icon="mdi:play"></ha-icon>${this._t("workflowStart")}</button><button class="ghost" data-task-id="${task.id}" data-workflow-state="open"><ha-icon icon="mdi:pause"></ha-icon>${this._t("workflowSetOpen")}</button><button class="ghost" data-task-id="${task.id}" data-workflow-state="blocked"><ha-icon icon="mdi:pause-octagon-outline"></ha-icon>${this._t("workflowBlock")}</button><button class="ghost" data-reset-task-progress="${task.id}"><ha-icon icon="mdi:restart"></ha-icon>${this._t("workflowResetAction")}</button><button class="ghost" data-snooze-menu="${task.id}"><ha-icon icon="mdi:clock-plus-outline"></ha-icon>${this._t("snooze")}</button><button class="ghost" data-edit="${task.id}"><ha-icon icon="mdi:pencil"></ha-icon>${this._t("edit")}</button><button class="ghost" data-action="open-task-detail" data-task-id="${task.id}"><ha-icon icon="mdi:card-text-outline"></ha-icon>${this._t("openDetails")}</button>`;
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _qualityDialogHtml() {
    ...
    }).join("")}</div></div><footer><button class="ghost" data-action="close-quality-dialog">${this._t("cancel")}</button>${issues.length ? `<button class="ghost" data-action="filter-quality-issues"><ha-icon icon="mdi:filter-variant"></ha-icon>${this._t("filterQualityIssues")}</button>` : ""}</footer></section></div>`;
  },

  _filterQualityIssues() {
    this._qualityDialogOpen = false;
    this._view = "dashboard";
    this._search = "quality:issue";
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _templateCard(t) {
    ...
    const badges = [
      ...
      t.custom ? `<span class="template-badge">${this._t("customTemplateBadge")}</span>` : "",
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  _settingsIndex() {
    return [
      ...
      ["platforms", ["todoPlatform", "calendarPlatform", "calendarDuration", "nativePlatforms"]],
      ["manual", ["sortPosition"]],
      ["notifications", ["notifyService", "digestTime", "repeatEveryDays", "escalationAfterDays", "actionSnoozeDays", "quietHours", "dailyDigest", "actionableNotifications"]],
    ].flatMap(([tab, keys]) => keys.map(key => ({ tab, key, label: this._t(key) })));
  },

  _settingsSearchResultsHtml() {
    const query = String(this._settingsSearch || "").trim().toLowerCase();
    if (!query) return "";
    const matches = this._settingsIndex().filter(entry => entry.label.toLowerCase().includes(query));
    if (!matches.length) return `<p class="section-hint settings-search-results">${this._t("settingsSearchEmpty")}</p>`;
    const tabLabel = { general: "general", templates: "templates", data: "dataSafety", workflow: "workflow", platforms: "nativePlatforms", manual: "sortPosition", notifications: "notifications" };
    return `<div class="settings-search-results">${matches.slice(0, 12).map(entry => `<button class="ghost small" data-settings-jump="${entry.tab}"><strong>${this._html(entry.label)}</strong><small>${this._t(tabLabel[entry.tab])}</small></button>`).join("")}</div>`;
  },
  ...
  _settingsHtml() {
    ...
    const tabs = [
      ...
      ["templates", "mdi:shape-outline", this._t("templates")],
    ];
    const activeTab = ["general", "templates", "data", "workflow", "platforms", "manual", "notifications"].includes(this._settingsTab) ? this._settingsTab : "general";
    return `
      ...
      <label class="field settings-search-field"><span>${this._t("settingsSearch")}</span><input id="settingsSearch" class="search" value="${this._html(this._settingsSearch)}" placeholder="${this._t("settingsSearch")}"></label>
      ${this._settingsSearchResultsHtml()}
      ...
      <section class="settings-stack">
        ${activeTab === "general" ? `<article class="panel settings-section" id="dashboard-settings">
          ...
          <div class="settings-subpanel"><h4>${this._t("statisticsCosts")}</h4><p class="section-hint">${this._t("costTrackingHint")}</p><div class="check-grid"><label class="check"><input id="dashboardCostTracking" type="checkbox" ${dashboard.cost_tracking ? "checked" : ""}>${this._t("costTracking")}</label></div><div class="form-grid"><label class="field"><span>${this._t("defaultCurrency")}</span><select id="dashboardCurrency">${CURRENCIES.map(code => `<option value="${code}" ${(dashboard.default_currency || "EUR") === code ? "selected" : ""}>${code}</option>`).join("")}</select></label><label class="field"><span>${this._t("numberFormat")}</span><select id="dashboardNumberFormat">${[["auto","numberFormatAuto"],["comma","numberFormatComma"],["dot","numberFormatDot"],["space","numberFormatSpace"]].map(([value,label]) => `<option value="${value}" ${(dashboard.number_format || "auto") === value ? "selected" : ""}>${this._t(label)}</option>`).join("")}</select><small class="field-hint">${this._t("numberFormatHint")}</small></label></div></div>
          ${this._budgetSettingsHtml()}
        ...
        ${activeTab === "templates" ? `<article class="panel settings-section templates-settings-section" id="template-settings">${this._templatesSectionHtml()}</article>` : ""}
        ...
        ${activeTab === "workflow" ? `<article class="panel settings-section" id="workflow-settings">
          ...
          <div class="settings-section-grid two-column">
            ...
            <div class="settings-subpanel"><h4>${this._t("completionRequirements")}</h4><div class="check-grid">
              ...
              <label class="check ${dashboard.cost_tracking ? "" : "is-muted"}" title="${dashboard.cost_tracking ? "" : this._t("costTrackingHint")}"><input id="workflowRequireCost" type="checkbox" ${workflowDefaults.cost ? "checked" : ""} ${dashboard.cost_tracking ? "" : "disabled"}>${this._t("completionRequirementCost")}</label>
        ...
        ${activeTab === "notifications" ? `<article class="panel settings-section" id="notification-settings">
          ...
          <div class="settings-section-grid two-column">
            <div class="settings-subpanel"><h4>${this._t("notificationRules")}</h4><div class="form-grid">${this._notifyServiceField("notifyService", notifications.notify_service)}<label class="field"><span>${this._t("digestTime")}</span><input id="digestTime" type="time" value="${this._html(notifications.digest_time || "08:00")}"></label><label class="field"><span>${this._t("repeatEveryDays")}</span><input id="notificationRepeatDays" type="number" min="0" max="365" value="${this._html(notifications.repeat_days ?? 3)}"></label><label class="field"><span>${this._t("escalationAfterDays")}</span><input id="notificationEscalationDays" type="number" min="0" max="365" value="${this._html(notifications.escalation_after_days ?? 3)}"></label><label class="field"><span>${this._t("actionSnoozeDays")}</span><input id="actionSnoozeDays" type="number" min="1" max="365" value="${this._html(notifications.action_snooze_days ?? 7)}"></label><label class="field"><span>${this._t("notificationHistoryRetention")}</span><input id="notificationHistoryRetention" type="number" min="20" max="2000" value="${this._html(notifications.history_retention ?? 200)}"></label></div></div>
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _historyHtml() {
    ...
    const rows = grouped.map(([day, items]) => `<section class="history-day-group"><div class="history-day-header"><strong>${this._historyDayLabel(day)}</strong><span>${items.length}</span></div><div class="history-day-entries history-axis">${items.map(event => {
      const completion = event.details?.completion || event.details || {};
      ...
      const completionBits = [
        ...
        completion.cost != null && this._costTrackingEnabled() ? `<span><ha-icon icon="mdi:cash"></ha-icon>${this._html(completion.cost)} ${this._html(completion.currency || this._defaultCurrency())}</span>` : "",
        ...
        this._attachmentLinksHtml(completion.attachments),
      ].filter(Boolean).join("");
      const changes = this._historyChanges(event);
      const before = event.details?.runtime_before ? this._runtimeSummary(event.details.runtime_before, this._historyTaskFor(event)) : "";
      return `<article class="history-row ${this._html(event.type || "event")}"><div class="history-marker"><ha-icon icon="${this._historyIcon(event.type)}"></ha-icon></div><div class="history-content"><div class="history-title"><strong>${this._html(event.task_name || event.task_id || this._t("globalLabel"))}</strong><div class="history-meta-strip"><span class="history-event-type">${this._historyEventLabel(event.type)}</span>${source ? `<span class="history-source">${this._html(source)}</span>` : ""}<span class="history-time">${this._time(event.created_at)}</span></div></div><p class="history-summary-line">${this._historySummaryLine(event)}</p>${before ? `<small class="history-runtime-before">${before}</small>` : ""}${completionBits ? `<div class="completion-details">${completionBits}</div>` : ""}${changes}</div>${event.type === "completed" && !event.undone_at ? `<button class="ghost" data-undo="${event.id}">${this._t("undo")}</button>` : ""}</article>`;
  ...
  // Named *For to avoid colliding with the _historyTask filter property.
  _historyTaskFor(event) {
    return (this._state?.tasks || []).find(task => task.id === event.task_id);
  },

  // event.summary stays English in the store as a machine field; the UI always
  // renders from event.type so nothing untranslated reaches the user.
  _historySummaryLine(event) {
    ...
    const key = {
      ...
      reactivated: "summaryReactivated",
      imported: "summaryImported",
      snoozed: "summarySnoozed",
      snooze_cleared: "summarySnoozeCleared",
      undo_completed: "summaryUndoCompleted",
      workflow_reset: "workflowResetSummary",
      cycle_restarted: "cycleRestartedSummary",
      cycle_skipped: "cycleSkippedSummary",
    }[event.type];
    if (key) return this._html(this._t(key));
    if (String(event.type || "").startsWith("bulk_")) return this._html(this._t("summaryBulk"));
    return this._html(this._historyEventLabel(event.type));
  },
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _dialogHtml() {
    ...
    const d = this._draft;
    const pickPhase = this._isTemplatePickPhase();
    ...
    const stepHtml = pickPhase
      ? this._wizardTemplatePickHtml()
      : [
    ...
    const stepper = pickPhase ? "" : `<nav class="wizard-steps" aria-label="${this._t("newEntryWizard")}">${steps.map(([icon, label], index) => `<button class="${activeStep === index ? "active" : ""} ${this._dialogStepMissing(index).length ? "has-missing" : ""}" data-action="${index < activeStep ? "wizard-back" : "wizard-next"}" ${index === activeStep ? "disabled" : ""}><ha-icon icon="${icon}"></ha-icon><span>${this._t(label)}</span></button>`).join("")}</nav>`;
    const title = this._dialog === "edit" ? this._t("edit") : pickPhase ? this._t("createFromTemplate") : this._t("newEntry");
    const hint = pickPhase ? this._t("wizardTemplatePickHint") : missingTotal ? this._t("wizardMissingHint").replace("{count}", String(missingTotal)) : this._t("wizardReadyHint");
    const footer = pickPhase
      ? `<footer><button class="ghost" data-action="close">${this._t("cancel")}</button><button class="primary" data-action="confirm-template-pick" ${this._templateDraftId ? "" : "disabled"}><ha-icon icon="mdi:arrow-right"></ha-icon>${this._t("wizardContinueWithTemplate")}</button></footer>`
      : `<footer><button class="ghost" data-action="close">${this._t("cancel")}</button>${activeStep > 0 ? `<button class="ghost" data-action="wizard-back"><ha-icon icon="mdi:arrow-left"></ha-icon>${this._t("wizardBack")}</button>` : ""}${activeStep < steps.length - 1 ? `<button class="primary" data-action="wizard-next">${this._t("wizardNext")}</button>` : `<button class="primary" data-action="save" ${this._busy ? "disabled" : ""}>${this._t("save")}</button>`}</footer>`;
    return `<div class="dialog-backdrop"><section class="dialog wizard-dialog ${pickPhase ? "template-pick-dialog" : ""}"><header><div class="dialog-title-block"><h2>${title}</h2><p class="section-hint">${hint}</p></div><button class="icon" data-action="close"><ha-icon icon="mdi:close"></ha-icon></button></header>${stepper}<div class="dialog-body wizard-body">
      ${stepHtml}
      ${this._error ? `<div class="error">${this._html(this._error)}</div>` : ""}
      ${!pickPhase && this._duplicateTaskWarning ? `<div class="schedule-callout warning"><ha-icon icon="mdi:alert-outline"></ha-icon><div><strong>${this._t("duplicateTaskWarning")}</strong><p>${this._t("similarTaskExists")}: ${this._html(this._duplicateTaskWarning)}</p></div></div>` : ""}
    </div>${footer}</section></div>`;
  },

  _wizardTemplatePickHtml() {
    const picked = this._selectedTemplateDraft();
    const pickerQuery = this._templatePickerQuery || "";
    const options = this._wizardTemplateOptions();
    return `<section class="dialog-section wizard-section template-pick-section"><div class="template-autocomplete template-pick-only"><div class="template-picker-head"><label class="field grow"><span>${this._t("selectTemplate")}</span><input id="templatePicker" value="${this._html(pickerQuery)}" placeholder="${this._t("templateSearchPlaceholder")}" autocomplete="off"></label></div><p class="section-hint">${picked ? this._t("templatePickerSelected").replace("{name}", this._html(picked.name || "")) : this._t("wizardTemplatePickHint")}</p><div class="template-picker-list">${options.length ? options.map(t => `<button type="button" class="template-picker-option ${picked?.id === t.id ? "active" : ""}" data-pick-template="${this._html(t.id)}"><ha-icon icon="${this._html(t.icon || "mdi:wrench-clock")}"></ha-icon><span><strong>${this._html(t.name)}</strong><small>${this._categoryLabel(t)} · ${this._scheduleSummary(t)} · ${this._t("priority")} ${t.priority || 3}/5</small></span></button>`).join("") : `<p class="section-hint">${this._t("noTemplatesMatch")}</p>`}</div></div></section>`;
  },

  _wizardStartStepHtml(d) {
    const areas = this._areas();
    const appliedTemplate = this._createSource === "template" && !this._templatePickPhase ? this._selectedTemplateDraft() : null;
    const templateBanner = appliedTemplate ? `<div class="template-applied-banner"><ha-icon icon="${this._html(appliedTemplate.icon || "mdi:shape-outline")}"></ha-icon><div><strong>${this._t("templateAppliedFrom").replace("{name}", this._html(appliedTemplate.name || ""))}</strong><p class="section-hint">${this._t("templateAppliedHint")}</p></div><button class="ghost small" data-action="change-wizard-template" type="button"><ha-icon icon="mdi:swap-horizontal"></ha-icon>${this._t("changeTemplate")}</button></div>` : "";
    const startHint = this._createSource === "empty" ? this._t("wizardStartHintEmpty") : this._t("wizardStartHintFilled");
    return `<section class="dialog-section wizard-section"><div class="section-heading"><div><h3>${this._t("wizardStepStart")}</h3><p class="section-hint">${startHint}</p></div></div>
      ${templateBanner}
  ...
  // A checklist item doubles as a procedure step: instruction, estimated
  // duration and the part booked out when the step is ticked.
  _checklistEditorRow(item, index) {
    const parts = this._inventoryEnabled() ? (this._state?.parts || []) : [];
    return `<div class="checklist-editor-row">
      <label class="check compact"><input type="checkbox" data-draft-checklist-done="${index}" ${item.done ? "checked" : ""}>${this._t("done")}</label>
      <label class="field"><span>${this._t("label")}</span><input type="text" data-draft-checklist-label="${index}" value="${this._html(item.label || "")}" placeholder="${this._t("checklistItemPlaceholder")}"></label>
      <label class="check compact icon-check" title="${this._t("required")}"><input type="checkbox" data-draft-checklist-required="${index}" ${item.required ? "checked" : ""}><ha-icon icon="mdi:asterisk"></ha-icon></label>
      <button class="ghost icon-only" type="button" title="${this._t("delete")}" data-action="remove-checklist-item" data-checklist-index="${index}"><ha-icon icon="mdi:delete-outline"></ha-icon></button>
      <div class="checklist-step-details">
        <label class="field"><span>${this._t("stepHint")}</span><input type="text" data-draft-checklist-hint="${index}" value="${this._html(item.hint || "")}" placeholder="${this._t("stepHintPlaceholder")}"></label>
        <label class="field"><span>${this._t("stepDuration")}</span><input type="number" min="0" step="1" data-draft-checklist-duration="${index}" value="${item.duration_minutes ?? ""}"></label>
        ${parts.length ? `<label class="field"><span>${this._t("stepPart")}</span><select data-draft-checklist-part="${index}"><option value="">${this._t("stepNoPart")}</option>${parts.map(part => `<option value="${this._html(part.id)}" ${item.part_id === part.id ? "selected" : ""}>${this._html(part.name)}</option>`).join("")}</select></label>${item.part_id ? `<label class="field"><span>${this._t("stepQuantity")}</span><input type="number" min="0" step="0.001" data-draft-checklist-quantity="${index}" value="${item.quantity ?? ""}"></label>` : ""}` : ""}
      </div>
    </div>`;
  },
  ...
  _workflowFieldsHtml(d) {
    ...
    return `<section class="dialog-section wizard-section"><div class="section-heading"><div><h3>${this._t("workflow")}</h3><p class="section-hint">${this._t("workflowHint")}</p></div></div><div class="preset-row">${[["maintenance","mdi:wrench-clock","presetMaintenance"],["repair","mdi:alert-wrench-outline","presetRepair"]].map(([id, icon, label]) => `<button class="ghost" data-workflow-preset="${id}"><ha-icon icon="${icon}"></ha-icon>${this._t(label)}</button>`).join("")}</div><div class="form-grid"><label class="field"><span>${this._t("workflowState")}</span><select data-draft="workflow_state">${WORKFLOW_STATES.map(state => `<option value="${state}" ${d.workflow_state === state ? "selected" : ""}>${this._workflowStateLabel(state)}</option>`).join("")}</select></label></div><div class="checklist-editor"><div class="checklist-editor-head"><div><strong>${this._t("checklist")}</strong><p class="section-hint">${this._t("checklistHint")}</p></div><button class="ghost small" type="button" data-action="add-checklist-item"><ha-icon icon="mdi:plus"></ha-icon>${this._t("addChecklistItem")}</button></div>${checklist.length ? checklist.map((item, index) => this._checklistEditorRow(item, index)).join("") : `<p class="section-hint">${this._t("checklistEmpty")}</p>`}</div><div class="completion-requirements"><strong>${this._t("completionRequirements")}</strong><div class="check-grid">${[
      ...
      // The cost field is not rendered while cost tracking is off, so requiring
      // it would block completion with no way to satisfy the requirement.
      const blocked = key === "completion_requirements_cost" && !this._costTrackingEnabled();
      return `<label class="check ${blocked ? "is-muted" : ""}" title="${blocked ? this._t("costTrackingHint") : ""}"><input data-draft="${key}" type="checkbox" ${d[key] ? "checked" : ""} ${blocked ? "disabled" : ""}>${this._t(label)}</label>`;
  ...
  _diagnosticsHtml() {
    ...
    return `<div class="dialog-backdrop"><section class="dialog wide diagnostics-dialog"><header><div class="dialog-title-block"><h2>${this._t("diagnostics")}</h2><p class="section-hint">${this._t("settingsIntro")}</p></div><button class="icon" data-action="close-diagnostics"><ha-icon icon="mdi:close"></ha-icon></button></header><div class="dialog-body">
      <section class="dialog-section"><div class="diagnostic-grid">
        ...
        <div class="${storage.attachment_bytes > storage.attachment_limit_bytes * 0.8 ? "warning" : ""}"><span>${this._t("attachmentStorage")}</span><strong>${this._bytes(storage.attachment_bytes)} / ${this._bytes(storage.attachment_limit_bytes)}</strong><small>${storage.attachment_count || 0} · ${this._t("attachmentStorageHint")}</small></div><div><span>${this._t("healthTrend")}</span><strong>${storage.metrics_count || 0}</strong></div>
      </div></section>
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _templatePreviewHtml() {
    ...
    return `<div class="dialog-backdrop"><section class="dialog small"><header><h2>${this._t("preview")}</h2><button class="icon" data-action="close-template-preview"><ha-icon icon="mdi:close"></ha-icon></button></header><div class="dialog-body"><section class="dialog-section template-preview"><div class="template-preview-heading"><ha-icon icon="${this._html(t.icon)}"></ha-icon><div><h3>${this._html(t.name)}</h3><p>${this._html(t.description || "")}</p></div></div><div class="meta-grid"><div><span>${this._t("category")}</span><strong>${this._categoryLabel(t)}</strong></div><div><span>${this._t("schedule")}</span><strong>${this._scheduleSummary(t)}</strong></div><div><span>${this._t("priority")}</span><strong>${this._priorityLabel(t.priority)} ${t.priority}/5</strong></div></div><div class="preview-plan"><h3>${this._t("templateCreates")}</h3><ul><li>${this._t("schedule")}: ${this._scheduleSummary(t)}</li><li>${this._t("workflow")}: ${this._workflowStateLabel(t.workflow_state || this._defaultWorkflowState())}</li><li>${this._t("notifications")}: ${notifications.enabled === false ? this._t("disabled") : this._t("enabled")}</li>${checklist.length ? `<li>${this._t("checklist")}: ${checklist.length}</li>` : ""}</ul></div>${Array.isArray(t.tags) && t.tags.length ? `<div class="preview-tags"><span>${this._t("tags")}</span><div class="tag-list">${t.tags.map(tag => `<span>${this._html(tag)}</span>`).join("")}</div></div>` : ""}</section></div><footer><button class="ghost" data-action="close-template-preview">${this._t("cancel")}</button><button class="primary" data-template="${this._html(t.id)}"><ha-icon icon="mdi:plus"></ha-icon>${this._t("addTemplate")}</button></footer></section></div>`;
  }
});
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _onboardingDialogHtml() {
    ...
    return `<div class="dialog-backdrop"><section class="dialog onboarding-dialog"><header><div class="dialog-title-block"><h2>${this._t("onboarding")}</h2><p class="section-hint">${this._t("onboardingHint")}</p></div><button class="icon" data-action="skip-onboarding" title="${this._t("skip")}"><ha-icon icon="mdi:close"></ha-icon></button></header><div class="dialog-body"><section class="onboarding-hero"><div class="empty-orb"><ha-icon icon="mdi:home-wrench-outline"></ha-icon></div><div><h3>${this._t("onboardingTitle")}</h3></div></section><section class="pack-grid">${packs.map(pack => { const checked = this._selectedPacks.has(pack.id); return `<button type="button" class="pack-card ${checked ? "selected" : ""}" data-pack-toggle="${this._html(pack.id)}"><ha-icon icon="${this._html(pack.icon || "mdi:package-variant")}"></ha-icon><div><strong>${this._html(pack.name)}</strong><p>${this._html(pack.description || "")}</p><small>${(pack.template_ids || []).length} ${this._t("templates")}</small></div><span class="pack-check"><ha-icon icon="${checked ? "mdi:check-circle" : "mdi:circle-outline"}"></ha-icon></span></button>`; }).join("")}</section>
      <section class="dialog-section"><div class="section-heading"><div><h3>${this._t("modules")}</h3><p class="section-hint">${this._t("onboardingModulesHint")}</p></div></div>
        <div class="module-grid">${MODULE_KEYS.map(name => `<label class="check module-choice"><input data-onboarding-module="${name}" type="checkbox" ${this._selectedModules.has(name) ? "checked" : ""}><span><strong>${this._t(`module_${name}`)}</strong><small>${this._t(`module_${name}Hint`)}</small></span></label>`).join("")}</div>
      </section>
      <section class="dialog-section onboarding-notifications"><div class="section-heading"><div><h3>${this._t("onboardingNotifications")}</h3><p class="section-hint">${this._t("onboardingNotificationsHint")}</p></div></div>
        ${this._notifyServiceField("onboardingNotifyService", this._state?.settings?.notifications?.notify_service)}
        <label class="check"><input id="onboardingNotifyEnabled" type="checkbox" ${this._state?.settings?.notifications?.enabled ? "checked" : ""}>${this._t("enableNotifications")}</label>
      </section></div><footer><button class="ghost" data-action="skip-onboarding">${this._t("skip")}</button><button class="primary big" data-action="apply-onboarding"><ha-icon icon="mdi:rocket-launch-outline"></ha-icon>${this._t("startSetup")} · ${this._selectedPacks.size}</button></footer></section></div>`;
  }
});
  ...
  _notificationDialogHtml() {
    ...
    return `<div class="dialog-backdrop"><section class="dialog"><header><h2>${this._t("notifications")}</h2><button class="icon" data-action="close-notification-dialog"><ha-icon icon="mdi:close"></ha-icon></button></header><div class="dialog-body">
      ...
      <section class="dialog-section">
        ...
        <div class="form-grid">
          ${this._notifyServiceField("notifyService", n.notify_service || this._notifyService)}
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _bind() {
    ...
    onAll("[data-action='close-asset-dialog']", "click", () => { this._assetDraft = null; this._render(); });
    onAll("[data-action='save-asset']", "click", () => this._saveAsset());
    onAll("[data-action='delete-asset']", "click", () => this._deleteAsset());
    onAll("[data-action='propose-assets']", "click", () => this._proposeAssets());
    onAll("[data-action='close-asset-proposals']", "click", () => { this._assetProposals = null; this._render(); });
    onAll("[data-action='apply-asset-proposals']", "click", () => this._applyAssetProposals());
    onAll("[data-asset-field]", "input", el => { if (this._assetDraft) this._assetDraft[el.dataset.assetField] = el.value; });
    onAll("select[data-asset-field]", "change", el => { if (this._assetDraft) this._assetDraft[el.dataset.assetField] = el.value; });
    onAll("[data-action='open-report']", "click", () => this._openReport());
    onAll("[data-folder-toggle]", "click", el => {
      const id = el.dataset.folderToggle;
      if (this._openFolders.has(id)) this._openFolders.delete(id); else this._openFolders.add(id);
      this._render();
    });
    onAll("[data-document-filter]", "input", el => { this._documentFilter = el.value; this._renderSoon(); });
    onAll("[data-add-document]", "click", el => this._openDocumentDialog(null, el.dataset.addDocument));
    onAll("[data-edit-document]", "click", el => this._openDocumentDialog(el.dataset.editDocument));
    onAll("[data-action='close-document-dialog']", "click", () => { this._documentDraft = null; this._render(); });
    onAll("[data-action='save-document']", "click", () => this._saveDocument());
    onAll("[data-action='delete-document']", "click", () => this._deleteDocument());
    onAll("[data-document-field]", "input", el => { if (this._documentDraft) this._documentDraft[el.dataset.documentField] = el.value; });
    onAll("select[data-document-field]", "change", el => {
      if (!this._documentDraft) return;
      this._documentDraft[el.dataset.documentField] = el.value;
      if (el.dataset.documentField === "kind") this._render();
    });
    onAll("#documentFileInput", "change", el => this._pickDocumentFile(el.files?.[0]));
    onAll("#assetDeviceFilter", "input", el => { this._deviceFilter = el.value; this._renderSoon(); });
    onAll("[data-action='create-part']", "click", () => this._openPartDialog(null));
    onAll("[data-edit-part]", "click", el => this._openPartDialog(el.dataset.editPart));
    onAll("[data-action='close-part-dialog']", "click", () => { this._partDraft = null; this._render(); });
    onAll("[data-action='save-part']", "click", () => this._savePart());
    onAll("[data-action='delete-part']", "click", () => this._deletePart());
    onAll("[data-consume-part]", "click", el => this._consumePart(el.dataset.consumePart, Number(el.dataset.amount)));
    onAll("[data-part-field]", "input", el => { if (this._partDraft) this._partDraft[el.dataset.partField] = el.value; });
    onAll("select[data-part-field]", "change", el => { if (this._partDraft) this._partDraft[el.dataset.partField] = el.value; });
    onAll("[data-part-filter]", "input", el => { this._partFilter = el.value; this._renderSoon(); });
    onAll("[data-asset-contract]", "input", el => {
      if (!this._assetDraft) return;
      this._assetDraft.contract = { ...(this._assetDraft.contract || {}), [el.dataset.assetContract]: el.value };
    });
    ...
    on("settingsSearch", "input", e => { this._settingsSearch = e.target.value; this._render(); });
    onAll("[data-settings-jump]", "click", el => { this._settingsTab = el.dataset.settingsJump || "general"; this._settingsSearch = ""; this._persistUiState(); this._render(); });
    ...
    onAll("[data-action='open-onboarding']", "click", () => { this._onboardingDialog = true; this._selectedPacks.clear(); this._selectedModules.clear(); this._render(); });
    onAll("[data-onboarding-module]", "change", el => {
      if (el.checked) this._selectedModules.add(el.dataset.onboardingModule);
      else this._selectedModules.delete(el.dataset.onboardingModule);
    });
    ...
    onAll("[data-draft-checklist-hint]", "input", (el, event) => this._updateDraftChecklistItem(Number(el.dataset.draftChecklistHint), "hint", event.target.value));
    onAll("[data-draft-checklist-duration]", "input", (el, event) => this._updateDraftChecklistItem(Number(el.dataset.draftChecklistDuration), "duration_minutes", event.target.value));
    onAll("[data-draft-checklist-quantity]", "input", (el, event) => this._updateDraftChecklistItem(Number(el.dataset.draftChecklistQuantity), "quantity", event.target.value));
    onAll("[data-draft-checklist-part]", "change", (el, event) => this._updateDraftChecklistItem(Number(el.dataset.draftChecklistPart), "part_id", event.target.value));
    ...
    on("notifyService", "change", e => { this._notifyService = e.target.value; });
    ...
    onAll("[data-done]", "click", (el, e) => { e.preventDefault(); e.stopPropagation(); this._completeTask(el.dataset.done); });
    onAll("[data-done-details]", "click", (el, e) => { e.preventDefault(); e.stopPropagation(); this._completeTask(el.dataset.doneDetails, { withDetails: true }); });
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  async _applyOnboarding() {
    ...
    const notifyService = this.shadowRoot.getElementById("onboardingNotifyService")?.value;
    const notifyEnabled = this.shadowRoot.getElementById("onboardingNotifyEnabled")?.checked;
    await this.hass.callWS({
      type: "maintenance_dashboard/update_settings",
      patch: {
        onboarding: { completed: true, selected_packs: packIds },
        modules: {
          ...(this._state?.settings?.modules || {}),
          ...Object.fromEntries(MODULE_KEYS.map(name => [name, this._selectedModules.has(name)])),
        },
        ...(notifyService === undefined && notifyEnabled === undefined
          ? {}
          : { notifications: { notify_service: notifyService || "", enabled: Boolean(notifyEnabled) } }),
      },
    });
    this._onboardingDialog = false;
  ...
  _openAssetDialog(assetId) {
    const existing = (this._state?.assets || []).find(item => item.id === assetId);
    this._assetDraft = existing ? { ...existing, contract: { ...(existing.contract || {}) } } : { name: "", contract: {} };
    this._render();
  },
  ...
  async _saveAsset() {
    ...
    try {
      if (draft.id) await this.hass.callWS({ type: "maintenance_dashboard/update_asset", asset_id: draft.id, patch: draft });
      else await this.hass.callWS({ type: "maintenance_dashboard/create_asset", asset: draft });
      this._assetDraft = null;
      await this._load();
  ...
  async _deleteAsset() {
    const draft = this._assetDraft;
    if (!draft?.id || !window.confirm(this._t("deleteAssetHint"))) return;
    try {
      await this.hass.callWS({ type: "maintenance_dashboard/delete_asset", asset_id: draft.id });
      this._assetDraft = null;
      await this._load();
    } catch (error) {
      this._showToast(String(error));
  ...
  async _proposeAssets() {
    try {
      const result = await this.hass.callWS({ type: "maintenance_dashboard/propose_assets" });
      this._assetProposals = result?.proposals || [];
      this._render();
    } catch (error) {
      this._showToast(String(error));
    }
  ...
  async _applyAssetProposals() {
    const names = [...this.shadowRoot.querySelectorAll("[data-asset-proposal]")]
      .filter(el => el.checked).map(el => el.dataset.assetProposal);
    if (!names.length) return;
    try {
      const result = await this.hass.callWS({ type: "maintenance_dashboard/import_proposed_assets", names });
      this._assetProposals = null;
      await this._load();
      this._showToast(`${this._t("assetImportApply")}: ${result?.created || 0}`);
    } catch (error) {
      this._showToast(String(error));
    }
  },

  // The tab is opened before the await so the browser still credits it to the
  // click; opening it afterwards is treated as a popup and blocked.
  async _openReport() {
    const tab = window.open("", "_blank");
    try {
      const result = await this.hass.callWS({
        type: "maintenance_dashboard/build_report",
        year: Number(this._statisticsYear) || new Date().getFullYear(),
      });
      if (!tab) {
        this._showToast(this._t("openReportHint"));
        return;
      }
      tab.document.open();
  ...
  _openDocumentDialog(documentId, assetId) {
    const existing = (this._state?.media || []).find(item => item.id === documentId);
    this._documentDraft = existing ? { ...existing } : { kind: "invoice", asset_id: assetId || "", title: "", note: "" };
    this._render();
  },

  async _pickDocumentFile(file) {
    if (!file || !this._documentDraft) return;
    ...
    if (!data) { this._showToast(this._t("documentFileFailed")); return; }
    this._documentDraft.filename = file.name;
    this._documentDraft.mime_type = file.type || "application/octet-stream";
    this._documentDraft.data_base64 = data;
    this._render();
  },
  ...
  async _saveDocument() {
    ...
    try {
      if (draft.id) {
        await this.hass.callWS({
          type: "maintenance_dashboard/update_document",
          document_id: draft.id,
          patch: {
            kind: draft.kind, title: draft.title, note: draft.note,
            asset_id: draft.asset_id || "", reading: draft.reading, reading_unit: draft.reading_unit,
          },
        });
      } else {
        await this.hass.callWS({
          type: "maintenance_dashboard/store_document",
          asset_id: draft.asset_id || "",
          kind: draft.kind, title: draft.title || "", note: draft.note || "",
          reading: draft.reading === "" || draft.reading === undefined ? null : Number(draft.reading),
          reading_unit: draft.reading_unit || "",
          filename: draft.filename || "", mime_type: draft.mime_type || "",
  ...
  async _deleteDocument() {
    const draft = this._documentDraft;
    if (!draft?.id || !window.confirm(this._t("deleteDocumentHint"))) return;
    try {
      await this.hass.callWS({ type: "maintenance_dashboard/delete_document", document_id: draft.id });
      this._documentDraft = null;
      await this._load();
    } catch (error) {
      this._showToast(String(error));
  ...
  _openPartDialog(partId) {
    const existing = (this._state?.parts || []).find(item => item.id === partId);
    this._partDraft = existing ? { ...existing } : { name: "", unit: "Stk", stock: 0, minimum: 0 };
    this._render();
  },
  ...
  async _savePart() {
    ...
    if (!draft || !String(draft.name || "").trim()) return;
    try {
      await this.hass.callWS({ type: "maintenance_dashboard/save_part", part: draft });
      this._partDraft = null;
      await this._load();
      this._showToast(this._t("actionSaved"));
    } catch (error) {
  ...
  async _deletePart() {
    const draft = this._partDraft;
    if (!draft?.id || !window.confirm(this._t("deletePartHint"))) return;
    try {
      await this.hass.callWS({ type: "maintenance_dashboard/delete_part", part_id: draft.id });
      this._partDraft = null;
      await this._load();
    } catch (error) {
      this._showToast(String(error));
  ...
  async _consumePart(partId, amount) {
    try {
      await this.hass.callWS({ type: "maintenance_dashboard/consume_part", part_id: partId, amount });
      await this._load();
    } catch (error) {
      this._showToast(String(error));
    }
  ...
  async _confirmDone() {
    ...
    if (!id) return;
    const tracked = this._costTrackingEnabled();
    // Keep the dialog open when the backend rejects the completion, so the
    // entered note, material and checklist are not lost.
    const saved = await this._markDone(id, {
      note: this._completionNote,
      checklist: this._completionChecklist,
      material: this._completionMaterial,
      cost: tracked && this._completionCost !== "" ? Number(this._completionCost) : undefined,
      currency: tracked ? this._completionCurrency || this._defaultCurrency() : undefined,
      performed_by: this._completionPerformedBy,
      attachments: this._completionAttachments.map(item => item.id).filter(Boolean),
    });
    if (!saved) return;
    this._completionDialog = null;
    this._completionAttachments = [];
  ...
  async _sendNotification(test) {
    ...
    try {
      // Report the service that was actually used. Without a configured one the
      // backend falls back to persistent_notification, which is not a push.
      const result = test
        ? await this.hass.callWS({ type: "maintenance_dashboard/test_notification", service })
        : await this.hass.callWS({ type: "maintenance_dashboard/send_digest", service, include_snoozed: Boolean(this.shadowRoot.getElementById("includeSnoozed")?.checked) });
      await this._load();
      const target = result?.service || service;
      this._showToast(target ? `${this._t("actionNotificationSent")}: ${target}` : this._t("actionNotificationSent"));
    } catch (error) {
      this._showToast(String(error));
    }
  },
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  async _executeBulk() {
    ...
    if (this._bulkAction === "workflow") value = value || "open";
  ...
  async _updateTaskNote(taskId, noteId, text) {
    await this.hass.callWS({ type: "maintenance_dashboard/update_task_note", task_id: taskId, note_id: noteId, text });
    await this._load();
    this._showToast(this._t("noteSaved"));
  },

  async _deleteTaskNote(taskId, noteId) {
    await this.hass.callWS({ type: "maintenance_dashboard/delete_task_note", task_id: taskId, note_id: noteId });
    await this._load();
    this._showToast(this._t("noteDeleted"));
  },

  async _syncTemplateFavorites() {
    await this.hass.callWS({ type: "maintenance_dashboard/update_template_favorites", favorites: [...this._templateFavorites] });
  },

  async _toggleTemplateFavorite(id) {
    if (this._templateFavorites.has(id)) this._templateFavorites.delete(id);
    else this._templateFavorites.add(id);
    await this._syncTemplateFavorites();
    this._render();
  },

  async _saveTemplateFromTask(taskId) {
    await this.hass.callWS({ type: "maintenance_dashboard/save_template_from_task", task_id: taskId });
    await this._load();
    this._showToast(this._t("templateSaved"));
  },

  async _uploadCompletionAttachment(file) {
    if (!file) return;
    if (file.size > 512 * 1024) {
      this._showToast(this._t("attachmentTooLarge"));
      return;
    }
    ...
    const stored = await this.hass.callWS({
      type: "maintenance_dashboard/store_completion_attachment",
      attachment: {
        filename: file.name,
        mime_type: file.type || "application/octet-stream",
        data_base64: data,
        task_id: this._completionDialog,
        phase: this._completionPhase || "single",
      },
    });
    this._completionAttachments.push(stored);
    this._render();
  },

  async _previewTemplateImport() {
    const raw = String(this._templateImportPayload || "").trim();
    if (!raw) return;
    try {
      this._templateImportPreview = await this.hass.callWS({ type: "maintenance_dashboard/preview_user_template_import", payload: raw });
    } catch (error) {
      this._templateImportPreview = { valid: false, error: String(error) };
    }
    this._render();
  },

  async _importUserTemplates() {
    const raw = String(this._templateImportPayload || "").trim();
    if (!raw) return;
    const result = await this.hass.callWS({ type: "maintenance_dashboard/import_user_templates", payload: raw, skip_duplicates: true });
    this._templateImportOpen = false;
    this._templateImportPayload = "";
    this._templateImportPreview = null;
    await this._load();
    this._showToast(`${this._t("importTemplatesAction")}: ${result.added || 0}`);
  },

  async _saveCustomTemplate(template) {
    await this.hass.callWS({ type: "maintenance_dashboard/save_custom_template", template });
  },

  async _deleteCustomTemplate(templateId) {
    await this.hass.callWS({ type: "maintenance_dashboard/delete_custom_template", template_id: templateId });
  },

  _exportCustomTemplates() {
    const custom = (this._state?.templates || []).filter(template => template.custom);
    this._downloadJson({ version: VERSION, exported_at: new Date().toISOString(), templates: custom }, `maintenance-dashboard-templates-${new Date().toISOString().slice(0, 10)}.json`);
    this._showToast(this._t("exportTemplatesDone"));
  },

  async _openAttachment(attachmentId) {
    if (!attachmentId || !this.hass?.callWS) return;
    try {
      const record = await this.hass.callWS({ type: "maintenance_dashboard/get_attachment", attachment_id: attachmentId });
      const mime = record?.mime_type || "application/octet-stream";
      const data = record?.data_base64 || "";
      if (!data) return;
      ...
      anchor.href = url;
      anchor.download = record.filename || "attachment";
      anchor.click();
    } catch (error) {
      this._showToast(String(error));
    }
  ...
  async _saveGeneralSettings() {
    ...
    const patch = {
      dashboard: {
        ...
        widgets: this.shadowRoot.querySelector("[data-widget]")
          ? [...this.shadowRoot.querySelectorAll("[data-widget]")].filter(el => el.checked).map(el => el.dataset.widget)
          : currentDashboard.widgets,
        cost_tracking: this.shadowRoot.getElementById("dashboardCostTracking") ? Boolean(this.shadowRoot.getElementById("dashboardCostTracking")?.checked) : Boolean(currentDashboard.cost_tracking),
        default_currency: this.shadowRoot.getElementById("dashboardCurrency")?.value || currentDashboard.default_currency || "EUR",
        number_format: this.shadowRoot.getElementById("dashboardNumberFormat")?.value || currentDashboard.number_format || "auto",
      },
      modules: {
        ...(this._state?.settings?.modules || {}),
        ...Object.fromEntries([...this.shadowRoot.querySelectorAll("[data-module]")].map(el => [el.dataset.module, el.checked])),
      },
      budget: this.shadowRoot.querySelector("[data-budget]")
        ? Object.fromEntries([...this.shadowRoot.querySelectorAll("[data-budget]")]
            .filter(el => String(el.value).trim() !== "")
            .map(el => [el.dataset.budget, Number(el.value)]))
        : current.budget,
      backups: {
      ...
      workflow: {
        default_state: this.shadowRoot.getElementById("defaultWorkflowState")?.value || currentWorkflow.default_state || "open",
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _draftChange(e) {
    ...
    if (["category", "type", "schedule_mode", "calendar_repeat", "season", "priority", "notifications_inherit", "name"].includes(key)) {
      if (key === "name") this._duplicateTaskWarning = this._findDuplicateTask(this._draft)?.name || "";
      this._render();
    }
  },
  ...
  _defaultWorkflowState() {
    const state = this._workflowSettings().default_state || "open";
    return WORKFLOW_STATES.includes(state) ? state : "open";
  },
  ...
  _updateDraftChecklistItem(index, key, value) {
    ...
    if (!item) return;
    if (key === "done" || key === "required") item[key] = Boolean(value);
    else if (key === "duration_minutes" || key === "quantity") item[key] = value === "" ? null : Number(value);
    else item[key] = value;
    // The editor renders on every keystroke; re-rendering a text field would
    // move the caret to the end, so only checkboxes trigger a repaint.
    if (key === "done" || key === "required" || key === "part_id") this._render();
  },
  ...
  _workflowStateLabel(state) {
    return this._t(`workflow_${state || "open"}`);
  },

  _workflowTransitionLabel(state) {
    return this._t(`workflowSet_${state || "open"}`);
  },
  ...
  _completionMissing(task) {
    ...
    // Skip the cost requirement while cost tracking is off, otherwise the field
    // it asks for is not rendered and the task can never be completed.
    if (requirements.cost && this._costTrackingEnabled() && this._completionCost === "") missing.push("completionRequirementCost");
  ...
  _resetCreateDraft() {
    ...
    this._draft = {
      ...EMPTY,
      workflow_state: this._defaultWorkflowState(),
  ...
  _openCreateEmpty() {
    this._quickCreateOpen = false;
    this._createSource = "empty";
    this._templatePickPhase = false;
    this._dialogStep = 0;
    this._templateDraftId = "";
    this._templatePickerQuery = "";
    this._duplicateTaskWarning = "";
    this._resetCreateDraft();
    this._dialog = "create";
    this._render();
  },
  ...
  _openCreateFromTemplate() {
    this._quickCreateOpen = false;
    this._createSource = "template";
    this._templatePickPhase = true;
    this._dialogStep = 0;
    this._templateDraftId = "";
    this._templatePickerQuery = "";
    this._duplicateTaskWarning = "";
  ...
  _openCreate(template) {
    this._quickCreateOpen = false;
    this._createSource = template ? "template" : "empty";
    this._templatePickPhase = false;
    this._dialogStep = 0;
    this._templateDraftId = template?.id || "";
    this._templatePickerQuery = template?.name || "";
    ...
    this._duplicateTaskWarning = this._findDuplicateTask(this._draft)?.name || "";
    this._dialog = "create";
    this._render();
  },

  _isTemplatePickPhase() {
    return this._dialog === "create" && this._createSource === "template" && this._templatePickPhase;
  },

  _confirmTemplatePick() {
  ...
  _changeWizardTemplate() {
    if (this._dialog !== "create" || this._createSource !== "template") return;
    this._templatePickPhase = true;
    this._templateDraftId = "";
    this._templatePickerQuery = "";
    this._resetCreateDraft();
  ...
  _openEdit(id) {
    ...
    this._draft = {
      ...
      workflow_state: t.workflow_state || this._defaultWorkflowState(),
      asset_id: t.asset_id || "",
  ...
  _applyTemplate(t) {
    ...
    Object.assign(this._draft, {
      ...
      season: t.season || "autumn",
      tags: Array.isArray(t.tags) ? [...t.tags] : [],
      workflow_state: t.workflow_state || this._draft.workflow_state || this._defaultWorkflowState(),
    ...
    if (!this._draft.workflow_state) this._draft.workflow_state = this._defaultWorkflowState();
  },

  _wizardTemplateOptions() {
  ...
  _applyWorkflowPreset(preset) {
    if (preset === "persistent") {
      this._draft.workflow_state = "open";
  ...
  _dialogStepMissing(step = this._dialogStep) {
    const missing = [];
    if (step === 0 && this._isTemplatePickPhase()) {
      if (!this._templateDraftId) missing.push("selectTemplate");
      return missing;
    }
    if (step === 0) {
  ...
  _draftToTask() {
    ...
    return {
      ...
      workflow_state: this._draft.workflow_state || this._defaultWorkflowState(),
      asset_id: this._draft.asset_id || null,
      depends_on: this._draft.depends_on || null,
  ...
  // Nothing to ask for when no completion requirement and no checklist exist,
  // so skip the dialog. "Complete with details" forces it open.
  _needsCompletionDialog(task) {
    const requirements = this._completionRequirementsFromTask(task);
    const checklist = Array.isArray(task?.checklist) ? task.checklist : [];
    const showChecklist = this._workflowSettings().show_checklists !== false && checklist.length;
    // With evidence on, the dialog is the only place a photo can be attached.
    return Boolean(showChecklist || this._documentsEnabled() || Object.values(requirements).some(Boolean));
  },

  async _completeTask(id, { withDetails = false } = {}) {
    const task = (this._state?.tasks || []).find(item => item.id === id);
    if (!withDetails && !this._needsCompletionDialog(task)) {
      await this._markDone(id);
      return;
    }
    this._openCompletion(id);
  },
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  // Real notify.* services registered in this Home Assistant instance.
  // Companion-app targets first, they are what people actually want.
  _notifyServices() {
    ...
    return Object.keys(services)
      ...
      .sort((a, b) => {
        const rank = value => (value.includes("mobile_app") ? 0 : 1);
        return rank(a) - rank(b) || a.localeCompare(b);
      });
  },

  // Renders a picker over the discovered services. A configured value that no
  // longer exists is kept as an option so saving does not silently drop it.
  _notifyServiceField(id, current, { draft = false, emptyKey = "notifyServiceNone", disabled = "", labelKey = "notifyService" } = {}) {
    const services = this._notifyServices();
    const value = String(current || "");
    const options = [...services];
    if (value && !options.includes(value)) options.unshift(value);
    const attr = draft ? `data-draft="${id}"` : `id="${id}"`;
    const choices = [`<option value="">${this._t(emptyKey)}</option>`, ...options.map(service => {
      const missing = !services.includes(service);
      return `<option value="${this._html(service)}" ${service === value ? "selected" : ""}>${this._html(service)}${missing ? ` (${this._t("notifyServiceUnknown")})` : ""}</option>`;
    })].join("");
    const notificationsOff = !draft && value && this._state?.settings?.notifications?.enabled === false;
    const hint = !services.length
      ? `<small class="field-warning">${this._t("notifyServiceMissing")}</small>`
      : notificationsOff
        ? `<small class="field-warning">${this._t("notificationsDisabledWarning")}</small>`
        : `<small class="field-hint">${this._t("notifyServiceHint")}</small>`;
    return `<label class="field"><span>${this._t(labelKey)}</span><select ${attr} ${disabled}>${choices}</select>${hint}</label>`;
  },

  // Amounts are formatted through Intl so the separators follow the configured
  // locale instead of always being an English decimal point.
  _numberLocale() {
    return ({ comma: "de-DE", dot: "en-US", space: "fr-FR" })[this._state?.settings?.dashboard?.number_format]
      || (this._lang() === "de" ? "de-DE" : "en-US");
  },

  // Total estimated duration of a procedure; 0 means no step declared one.
  _procedureMinutes(checklist) {
    return (Array.isArray(checklist) ? checklist : [])
      .reduce((total, item) => total + (Number(item.duration_minutes) || 0), 0);
  },

  _num(value, maximumFractionDigits = 3) {
    return new Intl.NumberFormat(this._numberLocale(), { maximumFractionDigits }).format(Number(value) || 0);
  },

  _money(value, currency) {
    const amount = Number(value) || 0;
    const code = String(currency || this._defaultCurrency());
    try {
      return new Intl.NumberFormat(this._numberLocale(), { style: "currency", currency: code }).format(amount);
    } catch {
  ...
  _knownPerformers() {
    ...
    for (const event of this._state?.history || []) {
      ...
      const completion = event.details?.completion || event.details || {};
      const name = String(completion.performed_by || "").trim();
      if (name) names.set(name.toLowerCase(), name);
    }
    return [...names.values()].sort((a, b) => a.localeCompare(b, this._lang()));
  },

  _mediaUrl(attachmentId) {
    return `/api/maintenance_dashboard/media/${encodeURIComponent(attachmentId)}`;
  },

  _taskMedia(taskId) {
    const grouped = { before: [], after: [], single: [] };
    for (const item of this._state?.media || []) {
      if (item.task_id !== taskId) continue;
      (grouped[item.phase] || grouped.single).push(item);
    }
    return grouped;
  },
  ...
  _time(value) { if (!value) return "—"; return new Intl.DateTimeFormat(this._lang() === "de" ? "de-DE" : "en-US", { timeStyle: "short" }).format(new Date(value)); },
  ...
  // Renders a runtime snapshot taken *before* an action, so it is phrased in the
  // past tense and never shows a raw float.
  _runtimeSummary(runtime, task) {
    if (!runtime || runtime.remaining == null) return "";
    const unit = task?.schedule_mode && task.schedule_mode !== "interval" ? "days" : task?.interval_unit;
    const value = `${Math.ceil(Math.abs(runtime.remaining))} ${this._unitLabel(unit)}`;
    return this._t(runtime.remaining < 0 ? "wasOverdueBy" : "wasAheadBy").replace("{value}", value);
  },
  ...
  _activeDashboardFilters() {
    const chips = [];
    if (this._search.trim()) chips.push({ key: "search", label: `${this._t("search")}: ${this._search.trim()}` });
    if (this._statusFilter !== "all") chips.push({ key: "status", label: `${this._t("status")}: ${this._t(this._statusFilter)}` });
    if (this._categoryFilter !== "all") chips.push({ key: "category", label: `${this._t("category")}: ${this._t(this._categoryFilter)}` });
    if (this._areaFilter !== "all") chips.push({ key: "area", label: `${this._t("area")}: ${this._areaFilter}` });
    if (this._priorityFilter !== "all") chips.push({ key: "priority", label: `${this._t("priority")}: ${this._priorityFilter}` });
    if (this._scheduleFilter !== "all") chips.push({ key: "schedule", label: `${this._t("schedule")}: ${this._scheduleModeLabel(this._scheduleFilter)}` });
    if (this._dueFilter !== "all") chips.push({ key: "due", label: `${this._t("due")}: ${this._dueFilterLabel(this._dueFilter)}` });
    if (this._tagFilter.trim()) chips.push({ key: "tag", label: `${this._t("tags")}: ${this._tagFilter.trim()}` });
    if (this._entityFilter !== "all") chips.push({ key: "entity", label: `${this._t("entity")}: ${this._entityFilter === "missing" ? this._t("withoutEntity") : this._t("hasEntity")}` });
    if (this._showCompleted) chips.push({ key: "completed", label: this._t("showCompleted") });
    return chips;
  },

  _clearDashboardFilter(key) {
    if (key === "search") this._search = "";
    if (key === "status") this._statusFilter = "all";
    if (key === "category") this._categoryFilter = "all";
    if (key === "area") this._areaFilter = "all";
    if (key === "priority") this._priorityFilter = "all";
    if (key === "schedule") this._scheduleFilter = "all";
    if (key === "due") this._dueFilter = "all";
    if (key === "tag") this._tagFilter = "";
    if (key === "entity") this._entityFilter = "all";
    if (key === "completed") this._showCompleted = false;
    this._render();
  },

  _attachmentLinksHtml(attachments) {
    const items = Array.isArray(attachments) ? attachments : [];
    if (!items.length) return "";
    return `<div class="attachment-list">${items.map(item => `<button class="attachment-chip" data-open-attachment="${this._html(item.id || "")}"><ha-icon icon="mdi:paperclip"></ha-icon>${this._html(item.filename || item.id || this._t("attachment"))}</button>`).join("")}</div>`;
  },
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _styles() {
    ...
    @media (max-width:760px){.shell{padding:12px}.hero,.settings-head{flex-direction:column;align-items:stretch}.hero-brand{align-items:flex-start}.hero-logo{width:46px;height:46px}.empty-actions,footer,.actions{flex-direction:column;align-items:stretch}.task-grid,.template-grid{grid-template-columns:1fr}.dialog-backdrop{padding:8px}.snooze-menu{left:0;right:auto}}
    ...
    @media (max-width: 620px){.shell{padding:10px}.hero{gap:12px;border-bottom:0;margin-bottom:8px;padding-bottom:8px}.hero-brand{display:grid;grid-template-columns:42px 1fr;gap:10px}.hero-brand h1{font-size:1.35rem}.hero-brand p:not(.eyebrow){font-size:.8rem;line-height:1.25}.hero-actions{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}.hero-actions .nav{min-height:42px;padding:0 10px}.hero-actions .icon-nav{width:auto}.toolbar{padding:12px}.toolbar-main,.templates-main{grid-template-columns:1fr;gap:10px}.toolbar .primary.big,.templates-main .primary.big,.templates-main .search{grid-column:auto}.search{width:100%;min-width:0}.task-card{padding:14px;border-radius:22px}.task-card header{align-items:flex-start}.description{min-height:auto;font-size:.9rem}.meta-grid{grid-template-columns:1fr 1fr;gap:8px}.meta-grid div{padding:9px}.actions{display:grid!important;grid-template-columns:44px 44px 1fr;gap:8px}.actions .ghost:not(.icon-only){grid-column:1/-1}.actions .primary{min-width:0}.template-grid.compact{grid-template-columns:1fr}.template-card.compact{padding:14px}.template-card.compact footer{display:grid;grid-template-columns:1fr 1fr}.dialog-backdrop{align-items:end;place-items:end stretch;padding:0}.dialog{max-height:94vh;width:100%;border-radius:24px 24px 0 0}.dialog.small{width:100%}.dialog>header,.dialog>footer{padding:14px 16px}.dialog-body{padding:14px}.form-grid,.appearance-grid{grid-template-columns:1fr}.toast{left:10px;right:10px;bottom:10px}.category-tabs{margin:0 -4px;padding:2px 4px 12px}.settings-row{display:grid;grid-template-columns:28px 28px 1fr 36px 36px;gap:8px}.settings-row .danger{grid-column:5}.settings-actions{display:grid;grid-template-columns:1fr}.history-row{align-items:flex-start}.priority-head{display:grid;gap:6px}.priority-scale{grid-template-columns:1fr}.priority-scale span{text-align:left}.template-group{padding:14px}}
    ...
    .compact-brand{flex:0 1 auto;min-width:0;overflow:hidden}
    .hero-actions{height:100%;gap:20px;flex-wrap:nowrap;flex:0 0 auto;position:relative;z-index:2}
    ...
    .search,select,input,textarea{border-radius:var(--md-sys-shape-corner-large);border-color:var(--md-sys-color-outline-variant);background:var(--md-sys-color-surface-container-low);color:var(--md-sys-color-on-surface);min-height:56px;padding:0 20px;box-sizing:border-box;max-width:100%;}
    ...
    .quick-create-backdrop{position:fixed;inset:0;z-index:2147482895;background:rgb(0 0 0 / 10%)}
    .quick-create-menu{position:relative;z-index:2147482901;display:grid;gap:8px;padding:10px;border:1px solid var(--md-sys-color-outline-variant);border-radius:22px;background:var(--md-sys-color-surface-container-high);box-shadow:0 18px 42px rgb(0 0 0 / 28%)}
    ...
    .history-search-field{display:grid;gap:5px;min-width:0}
    .history-search-input{max-width:100%;width:100%}
    .history-toolbar label,.history-toolbar>div{min-width:0}

    .brand-version{margin-left:8px;padding:2px 8px;border-radius:999px;background:color-mix(in srgb,var(--primary-color) 18%,transparent);color:var(--secondary-text-color);font-size:.7rem;font-weight:800;letter-spacing:.02em;vertical-align:middle}

    .statistics-head{display:grid;grid-template-columns:auto 1fr;gap:12px;align-items:center;margin-bottom:14px}
    .statistics-head h2{margin:0;font-size:1.05rem}
    .statistics-head p{margin:2px 0 0;color:var(--secondary-text-color);font-size:.8rem}
    .statistics-panel+.statistics-panel{margin-top:14px}
    .stat-card-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:14px}
    .stat-card{display:grid;gap:2px;padding:12px 14px;border-radius:18px;background:color-mix(in srgb,var(--primary-text-color) 5%,transparent)}
    .stat-card span{color:var(--secondary-text-color);font-size:.72rem;font-weight:850;text-transform:uppercase;letter-spacing:.03em}
    .stat-card strong{font-size:1.45rem;line-height:1.1}
    .stat-card small{color:var(--secondary-text-color);font-size:.72rem}
    .statistics-columns{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:18px;align-items:start}
    ...
    .history-axis::before{content:"";position:absolute;left:9px;top:6px;bottom:6px;width:2px;border-radius:2px;background:color-mix(in srgb,var(--divider-color) 90%,transparent)}
    /* The marker is absolutely positioned, so it takes no grid column. Without
       this the content lands in the old 44px icon column and wraps per word. */
    .history-axis .history-row{position:relative;grid-template-columns:minmax(0,1fr) auto}
    .history-axis .history-row>button{align-self:start}
    .history-marker{position:absolute;left:-26px;top:2px;display:grid;place-items:center;width:20px;height:20px;border-radius:50%;background:var(--card-background-color);box-shadow:0 0 0 2px var(--divider-color)}
    .history-marker ha-icon{--mdc-icon-size:13px;color:var(--secondary-text-color)}
    .history-row.completed .history-marker{box-shadow:0 0 0 2px var(--success-color,#4caf50)}
    .history-row.completed .history-marker ha-icon{color:var(--success-color,#4caf50)}
    .history-row.deleted .history-marker{box-shadow:0 0 0 2px var(--error-color)}
    .history-runtime-before{color:var(--secondary-text-color);font-size:.76rem}
    .check.is-muted{opacity:.5;cursor:not-allowed}
    .field-warning{display:block;margin-top:6px;color:var(--warning-color);font-size:.74rem;font-weight:600;line-height:1.35}
    .field-hint{display:block;margin-top:6px;color:var(--secondary-text-color);font-size:.74rem;font-weight:500;line-height:1.35}
    .settings-search-field{display:grid;gap:5px;margin-bottom:10px;min-width:0}
    .settings-search-results{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px}
    .settings-search-results button{display:grid;gap:1px;text-align:left;padding:8px 12px}
    .settings-search-results small{color:var(--secondary-text-color);font-size:.7rem}
    .widget-grid{grid-template-columns:repeat(auto-fit,minmax(170px,1fr))}
    .asset-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:14px}
    .asset-card{padding:18px;display:grid;gap:12px;align-content:start}
    .asset-card-head{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:12px;align-items:center}
    .asset-card-head h3{margin:0;font-size:1.02rem}
    .asset-card-head p{margin:2px 0 0;color:var(--secondary-text-color);font-size:.8rem}
    .asset-expiries{display:flex;flex-wrap:wrap;gap:8px}
    .asset-task-list{display:flex;flex-wrap:wrap;gap:8px}
    .asset-task-list button{max-width:100%}
    .checklist-step-details{grid-column:1/-1;display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;padding-left:4px}
    .completion-step{display:grid;gap:4px}
    .step-hint{margin:0 0 0 30px;color:var(--secondary-text-color);font-size:.85rem;line-height:1.4}
    .step-meta{display:flex;flex-wrap:wrap;gap:12px;margin-left:30px;color:var(--secondary-text-color);font-size:.8rem}
    .step-meta span{display:inline-flex;align-items:center;gap:4px}
    .step-meta ha-icon{--mdc-icon-size:16px}
    .step-meta .step-short{color:var(--error-color)}
    .module-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:8px}
    .module-choice{align-items:flex-start;gap:10px}
    .module-choice span{display:grid;gap:2px;min-width:0}
    .module-choice small{color:var(--secondary-text-color);line-height:1.35}
    .folder-list{display:grid;gap:10px}
    .folder{padding:0;overflow:hidden}
    .folder-head{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;align-items:center;padding:10px 14px}
    .folder-toggle{display:grid;grid-template-columns:auto auto minmax(0,1fr);gap:10px;align-items:center;background:none;border:0;padding:0;text-align:left;color:inherit;min-width:0}
    .folder-title{display:grid;gap:2px;min-width:0}
    .folder-title small{color:var(--secondary-text-color);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .folder-count{min-width:26px;padding:2px 8px;border-radius:999px;text-align:center;background:color-mix(in srgb,var(--primary-text-color) 8%,transparent);font-size:.78rem;font-weight:800}
    .folder-body{display:grid;gap:12px;padding:0 14px 14px}
    .document-list{display:grid;gap:6px}
    .document-row{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:10px;align-items:center;padding:8px 10px;border-radius:14px;background:color-mix(in srgb,var(--primary-text-color) 5%,transparent)}
    .document-main{display:grid;gap:2px;min-width:0}
    .document-main a,.document-main span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .document-main small{color:var(--secondary-text-color);font-size:.78rem}
    .document-main p{margin:2px 0 0;color:var(--secondary-text-color);font-size:.84rem}
    .device-picker{display:grid;gap:8px}
    .device-picker select{width:100%;max-width:100%;min-height:auto;padding:8px;border-radius:14px}
    .part-actions{display:flex;flex-wrap:wrap;gap:8px}
    .restock-list{display:grid;gap:8px}
    .restock-row{display:grid;grid-template-columns:auto minmax(0,1fr) auto minmax(0,auto);gap:10px;align-items:center;padding:8px 12px;border-radius:16px;background:color-mix(in srgb,var(--warning-color) 12%,transparent);color:var(--warning-color)}
    .restock-row.critical{background:color-mix(in srgb,var(--error-color) 12%,transparent);color:var(--error-color)}
    .restock-row button{justify-self:start;color:inherit}
    .restock-row small{color:var(--secondary-text-color);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .budget-list{display:grid;gap:14px}
    .budget-row{display:grid;gap:6px}
    .budget-head{display:flex;justify-content:space-between;align-items:center;gap:12px}
    .budget-row small{color:var(--secondary-text-color)}
    .budget-bar{position:relative;height:12px;border-radius:999px;overflow:hidden;background:color-mix(in srgb,var(--disabled-text-color) 16%,transparent)}
    .budget-fill{height:100%;border-radius:999px;background:var(--success-color,#4caf50)}
    .budget-fill.warning{background:var(--warning-color)}
    .budget-fill.overdue{background:var(--error-color)}
    .budget-pace{position:absolute;top:0;bottom:0;width:2px;background:var(--primary-text-color);opacity:.55}
    @media(max-width:620px){.restock-row{grid-template-columns:auto minmax(0,1fr);grid-auto-rows:auto}.restock-row small{display:none}}
    .workflow-metric.blocked-by{color:var(--error-color);font-weight:700}
    .photo-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:10px;margin:8px 0 16px}
    .photo-tile{display:block;border-radius:14px;overflow:hidden;background:var(--md-sys-color-surface-container-high);aspect-ratio:1}
    .photo-tile img{width:100%;height:100%;object-fit:cover;display:block}
    .phase-switch{display:inline-flex;gap:4px;margin-bottom:8px}
    .phase-switch button{padding:6px 12px;border-radius:999px}
    .phase-switch button.active{background:var(--md-sys-color-secondary-container,color-mix(in srgb,var(--primary-color) 22%,transparent))}
    ...
    @media(max-width:760px){
      ...
      .hero-actions{gap:8px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));height:auto;}
    ...
    .template-pick-section{min-height:360px}
    .template-pick-only .template-picker-list{max-height:min(52vh,420px);grid-template-columns:1fr}
    .template-pick-dialog .wizard-body{padding-top:8px}
    .template-applied-banner{display:grid;grid-template-columns:auto 1fr auto;gap:12px;align-items:center;padding:14px 16px;border:1px solid color-mix(in srgb,var(--md-sys-color-primary) 35%,var(--md-sys-color-outline-variant));border-radius:18px;background:color-mix(in srgb,var(--md-sys-color-primary) 8%,var(--md-sys-color-surface-container))}
    .template-applied-banner ha-icon{--mdc-icon-size:28px;color:var(--md-sys-color-primary)}
    ...
    /* v2.3.0 consolidated toolbar, mobile sheet, detail tabs, statistics */
    .compact-dashboard-toolbar .dashboard-main{display:grid;grid-template-columns:minmax(280px,1fr) 150px 180px 48px minmax(180px,max-content);gap:10px;align-items:end}
    .compact-dashboard-toolbar .toolbar-field{display:grid;gap:4px;color:var(--md-sys-color-on-surface-variant);font-size:.72rem;font-weight:850}
    .compact-dashboard-toolbar .toolbar-field span{display:block}
    .compact-dashboard-toolbar .search,.compact-dashboard-toolbar select,.compact-dashboard-toolbar .toolbar-completed,.compact-dashboard-toolbar .toolbar-icon{height:48px;min-height:48px;box-sizing:border-box}
    .compact-dashboard-toolbar .toolbar-completed{display:inline-flex;align-items:center;gap:8px;justify-content:center;padding:0 14px;white-space:nowrap}
    .compact-dashboard-toolbar .layout-switch{min-width:0;align-self:end}
    .status-metric{display:inline-flex;align-items:center;gap:8px;color:var(--md-sys-color-on-surface-variant);cursor:default}
    .status-metric-action{border:0;background:transparent;padding:0;color:inherit;font:inherit;cursor:pointer;border-radius:999px}
    .status-metric-action:hover{background:color-mix(in srgb,var(--md-sys-color-primary) 10%,transparent)}
    .status-metrics-toggle{align-self:center}
    .detail-tabs{display:flex;gap:6px;overflow:auto;padding:0 0 12px;margin-bottom:8px}
    .detail-tabs button{display:inline-flex;align-items:center;gap:8px;min-height:40px;padding:0 12px;border:1px solid var(--md-sys-color-outline-variant);border-radius:999px;background:transparent;color:var(--md-sys-color-on-surface-variant);font-weight:800;white-space:nowrap}
    .detail-tabs button.active{background:var(--md-sys-color-primary);color:var(--md-sys-color-on-primary);border-color:var(--md-sys-color-primary)}
    .note-list article{display:grid;gap:6px;padding:12px;border:1px solid var(--md-sys-color-outline-variant);border-radius:16px;background:var(--md-sys-color-surface-container-low)}
    .note-head{display:flex;align-items:center;justify-content:space-between;gap:10px}
    .note-actions{display:flex;gap:4px}
    .mobile-sheet-backdrop{position:fixed;inset:0;z-index:2147483200;background:rgb(0 0 0 / 48%);display:grid;align-items:end}
    .mobile-action-sheet{width:100%;max-height:78vh;overflow:auto;padding:16px 16px calc(16px + env(safe-area-inset-bottom));border-radius:24px 24px 0 0;background:var(--md-sys-color-surface-container);border-top:1px solid var(--md-sys-color-outline-variant)}
    .mobile-action-sheet>header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px}
    .mobile-action-list{display:grid;gap:8px}
    .mobile-action-list button{width:100%;justify-content:flex-start;min-height:48px}
    .statistics-panel{display:grid;gap:18px}
    .statistics-summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}
    .statistics-summary article,.statistics-list article{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px;border:1px solid var(--md-sys-color-outline-variant);border-radius:18px;background:var(--md-sys-color-surface-container-low)}
    .statistics-summary span,.statistics-list small{display:block;color:var(--md-sys-color-on-surface-variant)}
    .statistics-section{display:grid;gap:10px}
    .quality-issue-list{display:grid;gap:10px}
    .quality-issue-row{display:grid;gap:10px;padding:12px;border:1px solid var(--md-sys-color-outline-variant);border-radius:16px;background:var(--md-sys-color-surface-container-low)}
    .quality-fix-actions{display:flex;gap:8px;flex-wrap:wrap}
    .active-filter-chips{display:flex;gap:8px;flex-wrap:wrap;width:100%}
    .active-filter-chip{display:inline-flex;align-items:center;gap:6px;min-height:34px;padding:0 10px;border-radius:999px;border:1px solid var(--md-sys-color-primary);background:color-mix(in srgb,var(--md-sys-color-primary) 12%,transparent);color:var(--md-sys-color-on-surface);font-weight:800}
    .page-header-actions{display:flex;gap:8px;flex-wrap:wrap}
    .platform-settings-card{border-color:color-mix(in srgb,var(--md-sys-color-primary) 35%,var(--md-sys-color-outline-variant))}
    .platform-status-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-bottom:14px}
    .platform-status-grid article{display:grid;grid-template-columns:auto 1fr auto;gap:12px;align-items:start;padding:14px;border:1px solid var(--md-sys-color-outline-variant);border-radius:18px;background:var(--md-sys-color-surface-container-low)}
    .platform-status-grid code{display:block;margin-top:6px;font-size:.78rem;color:var(--md-sys-color-primary)}
    .attachment-upload{display:grid;gap:10px;padding:12px;border:1px dashed var(--md-sys-color-outline-variant);border-radius:16px}
    .attachment-list,.attachment-chip{display:flex;gap:8px;flex-wrap:wrap}
    .attachment-chip{align-items:center;min-height:32px;padding:0 10px;border-radius:999px;background:var(--md-sys-color-surface-container-high);font-size:.82rem}
    .template-card header .favorite-button{margin-left:auto;flex:0 0 auto;position:relative;z-index:2}
    .template-title{flex:1;min-width:0;padding-right:4px}
    /* v2.4.0 statistics bars, shortcuts dialog, linkish buttons */
    .statistics-bars{display:grid;gap:10px}
    .statistics-bar-row{display:grid;grid-template-columns:minmax(120px,1fr) minmax(0,2fr) auto;gap:12px;align-items:center;padding:10px 12px;border:1px solid var(--md-sys-color-outline-variant);border-radius:16px;background:var(--md-sys-color-surface-container-low)}
    .statistics-bar-label strong,.statistics-bar-label small{display:block}
    .statistics-bar-label small{color:var(--md-sys-color-on-surface-variant);font-size:.76rem}
    .statistics-bar-track{height:10px;border-radius:999px;background:var(--md-sys-color-surface-container-highest);overflow:hidden}
    .statistics-bar-track span{display:block;height:100%;border-radius:999px;background:linear-gradient(90deg,var(--md-sys-color-primary),color-mix(in srgb,var(--md-sys-color-primary) 70%,var(--md-sys-color-tertiary)))}
    .statistics-task-row{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px;border:1px solid var(--md-sys-color-outline-variant);border-radius:16px;background:var(--md-sys-color-surface-container-low)}
    .statistics-year-field{min-width:120px}
    .shortcut-list{display:grid;gap:8px}
    .shortcut-row{display:grid;grid-template-columns:72px minmax(0,1fr);gap:12px;align-items:center;padding:10px 12px;border:1px solid var(--md-sys-color-outline-variant);border-radius:14px;background:var(--md-sys-color-surface-container-low)}
    .shortcut-row kbd{display:inline-grid;place-items:center;min-height:32px;padding:0 10px;border-radius:10px;border:1px solid var(--md-sys-color-outline-variant);background:var(--md-sys-color-surface-container-high);font-weight:900;font-family:inherit}
