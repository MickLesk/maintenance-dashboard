...
class MaintenanceDashboardPanel extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `${this._styles()}<main class="shell density-${this._html(this._density)}">${this._hero()}${content}${this._dialogHtml()}${this._taskDetailSheetHtml()}${this._qualityDialogHtml()}${this._templateImportDialogHtml()}${this._mobileActionSheetHtml()}${this._shortcutsDialogHtml()}${this._historyDialogHtml()}${this._diagnosticsHtml()}${this._dataDialogHtml()}${this._notificationDialogHtml()}${this._templatePreviewHtml()}${this._completionDialogHtml()}${this._bulkPreviewHtml()}${this._assetDialogHtml()}${this._onboardingDialogHtml()}${this._toastHtml()}</main>`;
    this._bind();
    ...
    return `
      ...
      <section class="settings-stack">
        ${activeTab === "general" ? `<article class="panel settings-section" id="dashboard-settings">
          ...
          <div class="settings-subpanel"><h4>${this._t("modules")}</h4><p class="section-hint">${this._t("modulesHint")}</p><div class="check-grid">${["assets","consumption","evidence"].map(name => `<label class="check" title="${this._t(`module_${name}Hint`)}"><input data-module="${name}" type="checkbox" ${(settings.modules || {})[name] ? "checked" : ""}>${this._t(`module_${name}`)}</label>`).join("")}</div></div>
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  async _uploadCompletionAttachment(file) {
    ...
    const stored = await this.hass.callWS({
      ...
      attachment: { filename: file.name, mime_type: file.type || "application/octet-stream", data_base64: data },
    });
