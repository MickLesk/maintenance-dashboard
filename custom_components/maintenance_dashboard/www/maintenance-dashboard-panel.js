...
const TEMPLATE_CATEGORY_KEYS = ["all", "recommended", "heating", "ventilation", "water", "electrical", "safety", "solar", "garden", "building", "it_network", "household", "garage", "seasonal"];
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _dialogHtml() {
    ...
    </div><footer><button class="ghost" data-action="close">${this._t("cancel")}</button>${activeStep > 0 ? `<button class="ghost" data-action="wizard-back"><ha-icon icon="mdi:arrow-left"></ha-icon>${this._t("previousTask")}</button>` : ""}${activeStep < steps.length - 1 ? `<button class="primary" data-action="wizard-next">${this._t("next")}</button>` : `<button class="primary" data-action="save" ${this._busy ? "disabled" : ""}>${this._t("save")}</button>`}</footer></section></div>`;
  },
  ...
  _wizardStartStepHtml(d) {
    ...
    return `<section class="dialog-section wizard-section"><div class="section-heading"><div><h3>${this._t("wizardStepStart")}</h3><p class="section-hint">${this._t("wizardStartHint")}</p></div></div>
      ${this._dialog === "create" ? `<div class="template-autocomplete"><label class="field grow"><span>${this._t("selectTemplate")}</span><input id="templatePicker" list="templatePickerOptions" value="${this._html(picked?.name || "")}" placeholder="${this._t("templatePickerPlaceholder")}"><datalist id="templatePickerOptions">${templateOptions.map(t => `<option value="${this._html(t.name)}">${this._html(this._categoryLabel(t))}</option>`).join("")}</datalist></label><button class="ghost" data-action="apply-picked-template" ${picked ? "" : "disabled"}><ha-icon icon="mdi:shape-plus-outline"></ha-icon>${this._t("applyTemplate")}</button></div>` : ""}
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  _styles() {
    ...
    .template-autocomplete{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:end;padding:12px;border:1px solid var(--md-sys-color-outline-variant);border-radius:18px;background:var(--md-sys-color-surface-container-low)}
