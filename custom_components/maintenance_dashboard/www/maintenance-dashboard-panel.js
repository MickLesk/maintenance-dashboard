...
class MaintenanceDashboardPanel extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `${this._styles()}<main class="shell">${this._hero()}${content}${this._dialogHtml()}${this._historyDialogHtml()}${this._diagnosticsHtml()}</main>`;
    this._bind();
  ...
  _nextKpi(task) {
    if (!task?.id) return this._kpi("mdi:calendar-clock", this._t("next"), "—");
    const name = this._shortTaskName(task.name || "—");
    const sub = task.remaining != null ? `${Math.ceil(Math.abs(task.remaining))} ${this._t("days")} ${task.remaining < 0 ? this._t("overdue") : this._t("remaining")}` : this._t("focusNextTask");
    return `<article class="kpi next-kpi" data-focus-task="${this._html(task.id)}" title="${this._t("nextTaskHint")}"><ha-icon icon="mdi:calendar-clock"></ha-icon><div><small>${this._t("next")}</small><strong>${this._html(name)}</strong><span>${this._html(sub)}</span></div></article>`;
  }
  ...
  _bind() {
    ...
    this.shadowRoot.querySelectorAll("[data-focus-task]").forEach(el => el.addEventListener("click", () => this._focusTask(el.dataset.focusTask)));
