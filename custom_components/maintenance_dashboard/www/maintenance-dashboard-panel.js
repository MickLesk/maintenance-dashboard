...
var e = "1.0.1", t = [
	...
	constructor() {
		super(), this.attachShadow({ mode: "open" }), this._view = "dashboard", this._state = null, this._draft = { ...c }, this._dialog = null, this._diagnostics = !1, this._search = "", this._statusFilter = "all", this._sortMode = "smart", this._dragTaskId = null, this._snoozeMenu = null, this._selectedTemplates = /* @__PURE__ */ new Set(), this._busy = !1, this._error = "", this._searchTimer = null;
	}
	set hass(e) {
	...
	_t(e) {
		return s[this._lang()][e] || s.en[e] || e;
	}
	_html(e) {
	...
	_render() {
		let e = this._captureFocus(), t = this._state ? this._viewHtml() : "<div class=\"loading\">Loading…</div>";
		this.shadowRoot.innerHTML = `${this._styles()}<main class="shell">${this._hero()}${t}${this._dialogHtml()}${this._diagnosticsHtml()}</main>`, this._bind(), this._restoreFocus(e);
	}
	_renderSoon(e = 180) {
		this._searchTimer && clearTimeout(this._searchTimer), this._searchTimer = setTimeout(() => {
			this._searchTimer = null, this._render();
		}, e);
	}
	_captureFocus() {
	...
	_restoreFocus(e) {
		if (!e?.id) return;
		let t = this.shadowRoot?.getElementById(e.id);
		t && requestAnimationFrame(() => {
			if (t.focus(), typeof t.setSelectionRange == "function" && e.start != null) try {
				t.setSelectionRange(e.start, e.end ?? e.start);
			} catch {}
		});
	}
	_hero() {
	...
	_taskCard(e) {
      ...
      <div class="meta-grid">
        ...
        <div><span>${this._t("priority")}</span><strong>${this._priorityLabel(e.priority)}<em>${e.priority}/5</em></strong></div>
      </div>
	...
	_bind() {
		...
		})), this.shadowRoot.querySelectorAll("[data-action='add-selected']").forEach((e) => e.addEventListener("click", () => this._addSelectedTemplates())), this.shadowRoot.querySelectorAll("[data-action='random-colors']").forEach((e) => e.addEventListener("click", () => {
			this._randomizeColors("both");
		})), this.shadowRoot.querySelectorAll("[data-action='random-icon-color']").forEach((e) => e.addEventListener("click", () => {
			this._randomizeColors("icon");
		})), this.shadowRoot.querySelectorAll("[data-action='random-card-color']").forEach((e) => e.addEventListener("click", () => {
			this._randomizeColors("card");
		})), this.shadowRoot.querySelectorAll("[data-action='clear-colors']").forEach((e) => e.addEventListener("click", () => {
			this._draft.icon_color = "", this._draft.card_color = "", this._render();
		}));
		let e = this.shadowRoot.getElementById("search");
		e && e.addEventListener("input", (e) => {
			this._search = e.target.value, this._renderSoon(240);
		});
	...
	_randomColor(e) {
		return e[Math.floor(Math.random() * e.length)] || "#00bcd4";
	}
	_randomizeColors(e = "both") {
		(e === "both" || e === "icon") && (this._draft.icon_color = this._randomColor(i)), (e === "both" || e === "card") && (this._draft.card_color = this._randomColor(a)), this._render();
	}
	_priorityLabel(e) {
		let t = Math.min(5, Math.max(1, Number(e) || 3));
		return o[this._lang()]?.[t] || o.en[t] || String(t);
	}
	_draftChange(e) {
...
customElements.get("maintenance-dashboard-panel") || customElements.define("maintenance-dashboard-panel", l), console.info(`%cmaintenance-dashboard-panel%c v${e}`, "color: var(--primary-color); font-weight: 800;", "color: inherit;");
//#endregion
