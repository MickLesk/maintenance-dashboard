...
Object.assign(I18N.de, {
  ...
  actionFilterDeleted: "Filter gelöscht", previewRequired: "Bitte den Import zuerst prüfen", confirmBulk: "Mehrfachaktion wirklich ausführen?"
});
Object.assign(I18N.en, {
  ...
  actionFilterDeleted: "Filter deleted", previewRequired: "Preview the import first", confirmBulk: "Run this bulk action?"
});
...
Object.assign(FRONTEND_CONTRACTS, {
...
class MaintenanceDashboardPanel extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `${this._styles()}<main class="shell">${this._hero()}${content}${this._dialogHtml()}${this._historyDialogHtml()}${this._diagnosticsHtml()}${this._dataDialogHtml()}${this._notificationDialogHtml()}${this._templatePreviewHtml()}${this._completionDialogHtml()}${this._bulkPreviewHtml()}${this._onboardingDialogHtml()}${this._toastHtml()}</main>`;
    this._bind();
...
Object.assign(MaintenanceDashboardPanel.prototype, {
  ...
  async _saveCurrentFilter() {
    ...
    const filter = { id: `filter_${Date.now().toString(36)}`, name, values: this._currentFilterPayload() };
    await this.hass.callWS({ type: "maintenance_dashboard/update_settings", patch: { dashboard: { saved_filters: [...current, filter] } } });
    this._savedFilterName = "";
    await this._load();
    this._showToast(this._t("actionFilterSaved"));
  },
  ...
  async _saveGeneralSettings() {
    ...
    const patch = {
      dashboard: { view_mode: this.shadowRoot.getElementById("dashboardViewMode")?.value || "cards", widgets },
      backups: {
