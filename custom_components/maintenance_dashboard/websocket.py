...
@websocket_api.websocket_command({vol.Required("type"): "maintenance_dashboard/mark_done", vol.Required("task_id"): str, vol.Optional("done_at"): str})
@callback
def websocket_mark_done(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_mark_done(msg["task_id"], msg.get("done_at")))
        except Exception as err:  # noqa: BLE001
