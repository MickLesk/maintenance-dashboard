...
def async_register_websocket(hass: HomeAssistant) -> None:
    ...
    websocket_api.async_register_command(hass, websocket_create_asset)
    websocket_api.async_register_command(hass, websocket_update_asset)
    websocket_api.async_register_command(hass, websocket_delete_asset)
    websocket_api.async_register_command(hass, websocket_propose_assets)
    websocket_api.async_register_command(hass, websocket_import_proposed_assets)
    websocket_api.async_register_command(hass, websocket_consumption_forecast)
    websocket_api.async_register_command(hass, websocket_save_part)
    websocket_api.async_register_command(hass, websocket_delete_part)
    websocket_api.async_register_command(hass, websocket_consume_part)
    websocket_api.async_register_command(hass, websocket_build_report)
    websocket_api.async_register_command(hass, websocket_store_document)
    websocket_api.async_register_command(hass, websocket_update_document)
    websocket_api.async_register_command(hass, websocket_delete_document)
...
@websocket_api.websocket_command({
    ...
    vol.Required("task_id"): str,
    vol.Required("state"): vol.In({"open", "in_progress", "blocked", "planned", "ready"}),
    vol.Optional("note"): str,
})
@websocket_api.require_admin
@callback
...
@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/create_asset",
    vol.Required("asset"): dict,
})
@websocket_api.require_admin
@callback
def websocket_create_asset(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_create_asset(msg["asset"]))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "create_asset_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/update_asset",
    vol.Required("asset_id"): str,
    vol.Required("patch"): dict,
})
@websocket_api.require_admin
@callback
def websocket_update_asset(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_update_asset(msg["asset_id"], msg["patch"]))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "update_asset_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/delete_asset",
    vol.Required("asset_id"): str,
})
@websocket_api.require_admin
@callback
def websocket_delete_asset(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_delete_asset(msg["asset_id"]))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "delete_asset_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/propose_assets",
})
@websocket_api.require_admin
@callback
def websocket_propose_assets(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_propose_assets())
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "propose_assets_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/import_proposed_assets",
    vol.Required("names"): [str],
})
@websocket_api.require_admin
@callback
def websocket_import_proposed_assets(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_import_proposed_assets(msg["names"]))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "import_assets_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/consumption_forecast",
    vol.Required("task_id"): str,
})
@callback
def websocket_consumption_forecast(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_consumption_forecast(msg["task_id"]))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "consumption_forecast_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/save_part",
    vol.Required("part"): dict,
})
@websocket_api.require_admin
@callback
def websocket_save_part(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_save_part(msg["part"]))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "save_part_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/delete_part",
    vol.Required("part_id"): str,
})
@websocket_api.require_admin
@callback
def websocket_delete_part(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_delete_part(msg["part_id"]))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "delete_part_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/consume_part",
    vol.Required("part_id"): str,
    vol.Required("amount"): vol.Coerce(float),
})
@callback
def websocket_consume_part(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_consume_part(msg["part_id"], msg["amount"]))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "consume_part_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/build_report",
    vol.Optional("year"): int,
})
@callback
def websocket_build_report(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            document = await _manager(hass).async_build_report(
                msg.get("year"), refresh_token_id=connection.refresh_token_id
            )
            connection.send_result(msg["id"], {"html": document})
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "build_report_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/store_document",
    vol.Optional("asset_id"): str,
    vol.Optional("task_id"): str,
    vol.Optional("kind"): str,
    vol.Optional("title"): str,
    vol.Optional("note"): str,
    vol.Optional("reading"): vol.Any(None, vol.Coerce(float)),
    vol.Optional("reading_unit"): str,
    vol.Optional("filename"): str,
    vol.Optional("mime_type"): str,
    vol.Optional("data_base64"): str,
})
@callback
def websocket_store_document(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_store_document(msg))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "store_document_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/update_document",
    vol.Required("document_id"): str,
    vol.Required("patch"): dict,
})
@callback
def websocket_update_document(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_update_document(msg["document_id"], msg["patch"]))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "update_document_failed", str(err))
    hass.async_create_task(_run())


@websocket_api.websocket_command({
    vol.Required("type"): "maintenance_dashboard/delete_document",
    vol.Required("document_id"): str,
})
@callback
def websocket_delete_document(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    async def _run() -> None:
        try:
            connection.send_result(msg["id"], await _manager(hass).async_delete_document(msg["document_id"]))
        except Exception as err:  # noqa: BLE001
            connection.send_error(msg["id"], "delete_document_failed", str(err))
    hass.async_create_task(_run())
