...
class MaintenanceTodoEntity(TodoListEntity):
    ...
    def todo_items(self) -> list[TodoItem]:
        ...
        include_disabled = bool(native_settings.get("todo_include_disabled", False))
    ...
    async def async_create_todo_item(self, item: TodoItem) -> None:
        ...
        task = await self.manager.async_create_task(payload)
        if item.status == TodoItemStatus.COMPLETED:
            note = translate(self.hass.config.language, "notifyCreatedFromTodo")
            await self.manager.async_mark_done(str(task["id"]), note=note)
    ...
    async def async_update_todo_item(self, item: TodoItem) -> None:
        ...
        if patch:
            task = await self.manager.async_update_task(str(item.uid), patch)
        runtime = self.manager.runtime_for_task(task)
        if item.status == TodoItemStatus.COMPLETED and runtime.status != "completed":
            note = translate(self.hass.config.language, "notifyCompletedFromTodo")
            await self.manager.async_mark_done(str(item.uid), note=note)
