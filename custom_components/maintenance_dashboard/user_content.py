...
def referenced_attachment_ids(history: list[dict[str, Any]]) -> set[str]:
...
def attachment_bytes(attachments: dict[str, Any]) -> int:
    return sum(
...
def prune_attachments(
    ...
    *,
    ...
    """Drop attachments whose history event rotated out, then enforce the cap.

    The store is rewritten in full on every save, so orphans would otherwise
    grow without bound.
    """
    kept = {key: value for key, value in attachments.items() if key in referenced}
    total = attachment_bytes(kept)
    if total <= max_bytes:
        return kept
    oldest_first = sorted(
        kept.items(),
    ...
    for attachment_id, record in oldest_first:
        if total <= max_bytes:
            break
        total -= int(record.get("size") or 0) if isinstance(record, dict) else 0
        kept.pop(attachment_id, None)
    return kept
