...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Record today's data point right away so the trend starts on the day the
    # integration is installed rather than a day later.
    try:
        await manager.async_record_health_snapshot()
    except Exception:  # noqa: BLE001
        _LOGGER.exception("Recording the initial maintenance health snapshot failed")
    ...
    async def _scheduled_notifications(_now) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Recording the maintenance health snapshot failed")
    ...
    async def _mobile_notification_action(event: Event) -> None:
        ...
        try:
            if action == "done":
                note = translate(hass.config.language, "notifyCompletedFromAction")
                await manager.async_mark_done(payload["task_id"], note=note)
            elif action == "snooze":
    ...
    await hass.config_entries.async_forward_entry_setups(
        entry, [Platform.SENSOR, Platform.BINARY_SENSOR, Platform.TODO, Platform.CALENDAR]
    )
