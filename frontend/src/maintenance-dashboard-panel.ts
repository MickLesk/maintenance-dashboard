...
class MaintenanceDashboardPanel extends HTMLElement {
  ...
  _taskCard(task) {
    ...
    return `<article class="task-card ${status}" style="--task-accent:${this._html(accent)}">
      ...
      <div class="meta-grid">
        ...
        <div><span>${this._t("priority")}</span><strong>${"●".repeat(Number(task.priority || 3))}<em>${task.priority}/5</em></strong></div>
      </div>
  ...
  _bind() {
    ...
    const search = this.shadowRoot.getElementById("search"); if (search) search.addEventListener("input", e => { this._search = e.target.value; this._render(); });
